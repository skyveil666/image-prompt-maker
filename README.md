...
AIアシスタントキャラが「次に何を変えるか」を提案してくれる。
...
- **AI分析エージェント**：履歴・お気に入り・組合せから具体的な改善提案＋ワンクリック反映
- **AIアシスタントキャラ**：左下に常駐するキャラが状況に合わせて声で提案（口調6種・感情6種・読み上げ対応）
...
`public/assistant-default.png` に好きな画像を置くと、アシスタントの既定アバターになります。
個別に変えたいだけならアプリ内 ⚙ → 📁 画像を選択（localStorage 保存）でもOK。
...
- アシスタントの位置自由配置（右下フローティング / ピン留めなど）
