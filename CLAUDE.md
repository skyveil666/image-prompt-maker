...
## 2. 最重要原則 P1-P8（master-plan §2 の要約）
...
**P8 自動注入は中立化に留める（主役導線を支配させない）**。
顔・同一性は **faceLock 単一管理**（LockKey に face/identity/expression を復活させない）。

**P8 自動注入は中立化に留める（主役導線を支配させない）**
自動注入（varietyEngine / promptSystem の語・設定を全案に挿入する仕組み）は
「多様性を出す」目的で強化しすぎると、ユーザーが何も選んでいなくても主役導線
（世界観プリセット・ストリート系等）で全開発火し、出力を支配する副作用を生む。

実害の記録：
- 退廃・廃墟・世紀末語の自動注入 → ChatGPT 画像フィルタでブロック（`2e6abea` #69）
- 劇的ポーズ（走り出す / 壁を蹴る）の強制注入 → 不自然な構図（`f3996e2` #71）
- 広角 / 魚眼の自動指定 → 等身崩れ（`f3996e2` #71）
（根：`530ddf1` の「変更を促す」回帰 = 変化強化のしすぎ）

禁則：
① 新しい自動注入を追加する前に「ユーザーが何も選んでいないのに全案に乗らないか」を確認する。
② 「変更を促す / 多様性を出す」系の強化は、主役導線（スコープ連動ブロック等）を支配しない強度に留める。
③ 修正は「削除」でなく「中立化 / 差し替え」を基本とする。
   棒立ち・無難への退行を避け、味を残しつつ毒だけ抜く（#69/#71 で実証済み）。
④ プロンプトへの注入は literal なトリガー語（廃墟 / 走り出す / 魚眼 等）より
   質感・空気感での表現を優先する。
