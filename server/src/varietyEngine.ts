...
const SURPRISE_TWISTS: readonly string[] = [
  ...
  "静かでミニマルな画面に、世紀末的・終末的な不穏さを一滴混ぜる",
  "監視カメラ（CCTV）／俯瞰の覗き見的アングルで切り取る視点の異化",
...
export function varietyBlock(plan: BatchPlan, req: GenerateRequest): string {
  ...
  if (req.scopes.includes("pose")) {
    lines.push("▼ ポーズ強化（静止立ち偏重を避け、動きのあるポーズを増やす）：");
    lines.push(
      "  しゃがみ込み / ジャンプ途中 / 振り向き / 歩行中 / 走り出し / 壁蹴り / 片足立ち /"
    );
    lines.push(
      "  寝転び / 座り込み / 見下ろし / 見上げ / 前傾姿勢 / バランス崩し / 大胆な手の演技"
    );
    lines.push("  各案でポーズの方向性を明確に変える。");
    lines.push("");
  }
