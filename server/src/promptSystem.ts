  ...
  if (scopes.includes("background")) {
    ...
    if (styleUnset && placeUnset) {
      ...
      lines.push(
        ...
        (bgBan.length
          ? "- 【絶対禁止（ユーザーNG指定・最優先・全案で一切使わない）】：" + bgBan.join("・") + "。\n" +
            "  これらユーザーNG指定は、上記【自然系】【室内系】【幻想系】等のどの採用指示よりも優先し、全案で確実に避ける。\n" +
            ((): string => {
              // 過剰除外防止の「除外しない」緩和は残すが、NG指定済みの語は緩和対象から外す（NG最優先）。
              const keep = ["海", "海岸", "空", "夕焼け", "雲海", "湖", "水辺", "花畑", "草原", "都市", "夜景", "室内（植物なし）"]
                .filter((w) => !bgBan.some((b) => w === b || w.includes(b)));
              return keep.length
                ? "  なお " + keep.join("・") + " 等のうち NG指定されていない背景は過剰除外しない（温室/植物が主役の背景のみ除外）。\n"
                : "";
            })()
          : "") +
