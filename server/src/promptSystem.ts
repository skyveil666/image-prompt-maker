  ...
  if (scopes.includes("background")) {
    ...
    if (styleUnset && placeUnset) {
      ...
      lines.push(
        ...
        "- 【室内系】" + trimCsv("スタジオ・白空間・黒空間・美術館・ホテル・ラウンジ・ガラス空間・未来室内") + " も選択肢に含める。\n" +
        "- 【幻想系】" + trimCsv("幻想空間・月夜・神秘空間・夢空間・ファンタジー背景") + " も積極的に採用する。\n" +
        (bgBan.length
          ...
          ? "- 【ユーザー除外指定あり・最優先】ユーザーが一部の背景（場所・スタイル）を除外指定している。該当する背景は全案で採用せず、上記【自然系】【室内系】【幻想系】等のどの採用指示よりも優先して避ける。\n" +
...
function avoidRealBackgroundBlock(): string {
  return [
    ...
    "背景の風景・空間は2D〜イラスト調で描く：アニメ背景／コンセプトアート／絵画的背景／2.5Dのスタイライズ空間。",
