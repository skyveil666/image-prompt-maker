...
function bodyFixBlock(): string {
  return [
    ...
    "─ 補正の狙い（やわらかい表現で書くこと）─",
    "・手は片手5本指で、指や腕のつくりが自然で破綻しないようにする。",
    "・余分な指・欠損した指・融合した指など、手の不自然さを自然に整える。",
    "・腕・脚・姿勢のバランスと、地面との自然な接地感を保つ。",
    "・影・接触影・重心バランスも自然に整える。",
    "─ 出力時の文例（そのまま使ってよい・やわらかい表現で書く）─",
    "【人体補正】手は片手5本指で自然な形。指・腕・脚のつくりと姿勢、接地のバランスが自然で破綻しない人体。" +
      " Natural, well-formed hands with five fingers each, natural limbs and balanced, grounded posture.",
  ].join("\n");
}

/**
 * 構図・人物サイズ補正ブロック。全案・全モード・全スコープで常時適用。省略不可。
 * 「人物が小さくなりすぎる／背景が主役化する」問題への恒久対策。
 *
 * ▼ 設計上の重要ルール（scopeFilter との干渉回避）:
 *   - 出力させる【構図・人物サイズ】行は「サイズ・主役性」の語のみで書かせる（俯瞰/画角/構図補助 等の
 *     カメラ scope 語を本文に入れさせない）。これにより camera スコープ未選択でも applyServerScopeFilter で削られない。
 *   - 文例に「維持」を必ず含めることで isProtective(line)=true となり、行ごと温存される（二重の保険）。
 *   - 背景の見た目・実写度・場所には一切言及しない（背景固定／avoidRealBackground を侵さない）。
 *   - 顔・年齢・人物造形・別人化の語を入れない（FACE_DANGER 非該当＝同一性保護と直交）。
 */
function subjectScaleBlock(): string {
  return [
    "【構図・人物サイズルール — 全案・全モード・全スコープ・絶対省略禁止】",
    "出力するすべての案に【構図・人物サイズ】セクションを必ず 1 行含める。このルールに例外はない。",
    "─ 原則 ─",
    "・人物（被写体）は常に画面内で主役の大きさを保ち、元画像と近いスケール感を維持する（同程度〜少しアップ〜少し引き程度まで）。",
    "・全身を入れる場合でも人物は十分大きく見せ、頭から足先までを自然に収めつつ主役として明確にする（目安：人物が画面高の 60〜90% を占める）。",
    "・上半身寄りの場合は、顔・髪・衣装・ポーズが読み取れる距離を保つ。",
    "・人物を遠景の点景・豆粒サイズにしない。極端な俯瞰・空撮・超遠距離・超広角で人物を小さくしない（どのアングル・どの距離でも人物のサイズを最優先）。",
    "・背景や巨大モチーフがどれだけ大きく・遠くても、人物の専有面積を主役として一定以上確保し、人物が背景に埋もれないようにする。",
    "─ 出力時の文例（そのまま使ってよい・サイズと主役性の表現のみで書く。カメラ用語は入れない）─",
    "【構図・人物サイズ】人物は元画像と近いスケール感を主役の大きさで維持し、豆粒化・遠景化させない。全身でも十分大きく、上半身でも顔がはっきり読める距離を保つ。",
  ].join("\n");
}

/**
 * 体型・頭身ブロック。全案・全モード・全スコープで常時適用。省略不可。
 * subjectScaleBlock（画面内での人物の「大きさ」）とは別に、人物“内部”の「比率（頭身・体型）」を守る。
 *
 * ▼ 設計上の重要ルール（scopeFilter / 同一性保護との干渉回避）:
 *   - 出力させる【体型・頭身】行は「比率・頭身」の語のみで書かせる（体型/頭身/顔/首/肩幅/胴体/脚の比率）。
 *     bodyShape は scopeFilter の CATEGORY_KEYWORDS に存在せず削除対象外。カメラ用語は入れさせない。
 *   - 文例に「維持」を必ず含めることで isProtective(line)=true となり、行ごと温存される（保険）。
 *   - 顔の造形・同一性を変える語（別人・輪郭を変える・目鼻立ちを変える 等の FACE_DANGER）は入れない。
 *     これは「頭身バランスのみ自然に整える」指示であり、顔の特徴・同一性の維持とは直交する。
 */
function bodyProportionBlock(): string {
  return [
    "【体型・頭身ルール — 全案・全モード・全スコープ・絶対省略禁止】",
    "出力するすべての案に【体型・頭身】セクションを必ず 1 行含める。このルールに例外はない。",
    "─ 原則（人物“内部”の比率を守る。画面内での大きさとは別の制約）─",
    "・元画像の人物比率を最優先で維持する。顔・首・肩幅・胴体・腕・脚の長さと太さのバランスを元画像に寄せる。",
    "・基本は8頭身前後の自然なファッションモデル体型。ただし厳密な固定ではなく、7〜9頭身の自然に見える範囲を許容する。",
    "・極端な脚長化、顔の小型化（小顔化しすぎ）、胴体の縮小、肩幅の誇張、別体型化は避ける。",
    "・全身でも上半身寄りでも、顔・首・胴・脚の比率を自然に保つ。モデル化しすぎて不自然にならないようにする。",
    "・魚眼・超広角・強い俯瞰や煽りで体が引き伸ばされても、人物の頭身・比率は元画像基準に補正する（構図の誇張より比率を優先）。",
    "・これは顔の造形・同一性を変える指示ではない。顔の特徴は維持したまま、頭身バランスのみ自然に整える。",
    "─ 出力時の文例（そのまま使ってよい・比率と頭身の表現のみで書く。カメラ用語は入れない）─",
    "【体型・頭身】元画像の人物比率を最優先で維持。基本は8頭身前後（広く見て7〜9頭身まで許容）の自然なモデル体型。顔・首・肩幅・胴体・脚の比率を不自然に伸縮させず、極端な脚長化・小顔化・胴体縮小・別体型化を避ける。",
  ].join("\n");
}
...
function poseDynamicsBlock(): string {
  return [
    ...
    "  上半身に角度をつけた動き / 走り出す直前 / 飛び降りる直前 / 片手を上に伸ばす /",
    "  体が傾く瞬間 / 服・髪が風になびく / 地面に片膝をつく / 颯爽と歩く /",
    "  振り返る途中の角度 / 肩ごしの視線 / 踊る途中のコマ止め",
...
function cameraAggressionBlock(): string {
  return [
    ...
    "  シネマティックなローアングル / 魚眼レンズ歪み / 斜め45度の見下ろし /",
    "  超ローアングル（地面すれすれ）/ 背後からの逆光シルエット /",
    "  超接近クローズアップ（顔の一部のみ）/ 斜め横エディトリアルアングル /",
    "  映画スチール風の斜め後方ショット / アーティスト写真風の横顔 /",
    "  鏡越し・ガラス越し反射 / 画面端に寄せた大胆トリミング /",
    "  走り抜ける瞬間のブレ（意図的モーションブラー）",
    "",
    "▼ 人物サイズ・体型比率は最優先で死守（アングル選択より優先）：",
    "  どんなに攻撃的・大胆なアングルでも、人物は画面内で主役の大きさを保ち、豆粒化・遠景の点景にしない。",
    "  極端な空撮・超遠距離・超広角で人物を小さくしない（アングルは攻めてよいが、人物のサイズは下げない）。",
    "  魚眼・超広角・強い俯瞰や煽りで体が歪んでも、頭身・体型比率は元画像基準に保つ（構図の誇張より人物の比率を優先）。",
    "",
    "▼ カメラを確定する前の確認：",
    "  □ 「正面バストアップ」「正面全身」がデフォルトになっていないか",
    "  □ このカメラアングル自体が「意外性」を持っているか",
    "  □ 各案でカメラアングルが明確に異なるか",
    "  □ アングルを攻めても人物が小さくなりすぎていないか",
...
export function safetySanitizePrompt(
  ...
  const rules: Array<[RegExp, string]> = [
    ...
    // ── 自傷・自殺の誤検知回避（ChatGPT 等の安全分類器が過剰反応する語を中立化）──
    // ファッション用途では無関係でも "wrist/手首" 等が自傷分類に誤反応するため、中立な語へ寄せる
    [/手首を切[^\s。、]*|リストカット/g,        "手元のデザイン"],
    [/手首/g,                                   "手元"],
    [/\bwrists?\b/gi,                           "hands"],
    ...
    // ── 盗撮・覗き見・パパラッチ系（非同意撮影イメージ） ────────────────────
    [/パパラッチ(?:望遠)?ショット?/g,          "映画スチール風ショット"],
    [/パパラッチ/g,                            "フォトジャーナリズム風"],
    [/覗き見的な?/g,                           "印象的な"],
    [/覗き見/g,                               "ユニークな視点からの"],
    [/盗撮/g,                                 "キャンドルショット"],
    ...
    [/ゴシックロリィタ|ゴシックロリータ|ゴスロリ/g, "ゴシックドール系"],
    [/スイートロリィタ|スイートロリータ/g,     "スイートドール系"],
    [/クラシカルロリィタ|クラシカルロリータ/g, "クラシカルドール系"],
    [/和ロリィタ|和ロリータ/g,               "和ドール系"],
    [/ロリィタ|ロリータ/g,                    "ドール系"],

    // ── 制服系ラベルバックストップ ────────────────────────────────────────
    [/JKストリート(?:風)?制服?/g,             "カレッジストリートスタイル"],
    [/JK制服/g,                              "カレッジスタイル"],
    [/セーラー服/g,                          "マリンカレッジウェア"],
...
export function buildSystemPrompt(
  ...
  const unifiedFormat = [
    ...
    "- 【構図・人物サイズ】人物を主役の大きさに保ち遠景化させない。1 行（簡潔・カメラ用語なし）。★省略禁止★",
    "- 【体型・頭身】元画像の人物比率を維持（7〜9頭身・脚長化/小顔化/別体型化を避ける）。1 行（簡潔）。★省略禁止★",
    "- 【人体補正】手は片手5本指、指・腕・脚と接地のバランスを自然に。1 行（簡潔・やわらかい表現）。★省略禁止★",
    ...
    "【構図・人物サイズ】人物は元画像と近いスケール感を主役の大きさで維持し、豆粒化・遠景化させない。全身でも十分大きく、上半身でも顔がはっきり読める距離を保つ。",
    "【体型・頭身】元画像の人物比率を最優先で維持。基本は8頭身前後（広く見て7〜9頭身）の自然なモデル体型。顔・首・肩幅・胴体・脚の比率を不自然に伸縮させず、極端な脚長化・小顔化・胴体縮小・別体型化を避ける。",
    "【人体補正】手は片手5本指で自然な形。指・腕・脚のつくりと姿勢・接地のバランスが自然で破綻しない人体。Natural, well-formed hands with five fingers each, natural limbs and balanced, grounded posture.",
    "---案区切り---",
    "【前提】... （案2 ここから）",
    "【固定】...",
    "（以下同様）",
  ...
  return [
    ...
    "\n" + subjectScaleBlock(),
    // ★ 体型・頭身ルールも常時挿入。条件なし。人物“内部”の比率（頭身・脚長・顔の大きさ・胴体）を守る。
