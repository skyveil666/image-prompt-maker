...
import { L3_COMPOUND } from "./gptFilterConstants.ts";
...
const HAIR_LABELS = {
  ...
  hairStyle: {
    ...
    magical_girl: "魔法少女風", gothic_lolita: "ゴシックドール系スタイル",
...
const COSPLAY_LABELS = {
  ...
  cuteStyle: {
    ...
    jirai: "地雷系", lolita: "ドール系", sweet_lolita: "スイートドール系",
  ...
  japaneseStyle: {
    ...
    wa_lolita: "和ドール系", taisho_western: "大正浪漫", japanese_bride: "和装花嫁",
  },
  fantasyStyle: {
  ...
  darkStyle: {
    gothic_lolita: "ゴシックドール系", vampire: "ゴシック系ヴァンパイアキャラクター", dark_witch: "神秘的な魔女",
  ...
  exposure: {
    ...
    bold:   "カバレッジ控えめ（肩・デコルテ・脚などの肌見せを活かした印象的なデザイン）",
  },
  colorDir: {
...
const OUTFIT_LABELS = {
  style: {
    ...
    y2k: "Y2K（2000年代初頭のポップスタイル）", lolita: "ドール系（フリルと可憐さを重視）",
  ...
  exposure: {
    ...
    normal:       "デザイン：標準シルエット",
    high:         "デザイン：カバレッジ控えめ（肩・デコルテ・脚などの肌見せを活かした、タンクトップ・ショート丈水準の開放的なデザイン）",
...
const BG_LABELS = {
  // 場所のプロンプト文：6/3基準の実空間名へ復元（元画像の実写質感を維持する方針）
  place: {
...
function describeDetails(
  ...
  if (scopes.includes("cosplay")) {
    ...
    lines.push(
      ...
      "露出はユーザー設定に委ねる。児童的・性的に露骨な方向のみ避け、それ以外はキャラクター表現として選べる。" +
  ...
  if (scopes.includes("background")) {
    ...
    if (bgStyleVals.length >= 2)
      ...
      fixed.push(`【背景スタイル】：${BG_LABELS.style[b.style as keyof typeof BG_LABELS.style]}（この作風を軸として保持しつつ、案ごとに表現・質感を変えて展開する）`);
    ...
    if (bgPlaceVals.length >= 2) {
      ...
      fixed.push(`場所：${BG_LABELS.place[b.place as keyof typeof BG_LABELS.place]}（この場所を軸として保持しつつ、案ごとに見せ方・要素・光を変えて展開する）`);
    ...
    if (styleUnset && placeUnset) {
      ...
      lines.push(
        ...
        "- 【自然系】" + trimCsv("森・花畑・海辺・空・雲海・雨上がり・湖・水辺・渓谷・草原") + " を積極的に採用する。\n" +
        "- 【作風系】" + trimCsv("現代アート・水彩・水墨・油絵・コンセプトアート") + " の作風で描いてよいが、単色・グラデーションだけの平面に逃げず立体感を残す。\n" +
        "- 【室内系】" + trimCsv("スタジオ・美術館・ホテルのロビー・ラウンジ・アトリエ・図書館・書斎") + " など奥行きのある室内も選択肢に含める。\n" +
        "- 【情景系】" + trimCsv("月夜の情景・霧の森・光の射す回廊・並木道・遠景の街並み・水辺の反射") + " など奥行きと物語性のある場面も積極的に採用する。\n" +
        "- いずれの方向でも、単色・グラデーションだけののっぺりした平面背景にせず、光と陰影・前後の層・奥行きを必ず持たせる。\n" +
        (bgBan.length
          // ★案P（pink-elephant解消）：NG済み場所を具体名で列挙するとGeminiが出力【NG】へecho→下流画像で
          //   逆に出やすくなる（2065「NG語を羅列せずポジティブ指示で書く」原則を背景banにも適用）。
          //   よって具体名は出さず、カテゴリ抽象で「除外指定あり・避けよ」とだけ伝える。実避けは
          //   varietyEngineのgenre除外＋上記の非NG候補採用で担保する。
          ? "- 【ユーザー除外指定あり・最優先】ユーザーが一部の背景（場所・スタイル）を除外指定している。該当する背景は全案で採用せず、上記の各採用指示よりも優先して避ける。\n" +
            "  ★NG指定された具体的な場所名は出力プロンプトの【NG】セクションに列挙しないこと（場所名を出力本文に残すと、下流の画像生成でその場所がかえって出やすくなる＝逆効果のため）。本文【変更】では、除外指定されていない上記候補から別方向の背景を積極的に選ぶことで避ける。\n"
          : "") +
  ...
  if (scopes.includes("foreground")) {
    ...
    const fgEffectVals = getMultiVals(details.multiOverrides, "foreground.effectType");
    if (fgEffectVals.length >= 2) fixed.push(`エフェクト種類：${fgEffectVals.map(v => fgl("effectType", v)).join("・")}（複数を重ねる）`);
    else if (fg.effectType !== "auto" && fg.effectType !== "skip") fixed.push(`エフェクト種類：${fgl("effectType", fg.effectType)}（固定）`);
    else if (fg.effectType === "auto") vary.push("エフェクト種類");
    const fgSwirlVals = getMultiVals(details.multiOverrides, "foreground.swirlType");
    if (fgSwirlVals.length >= 2) fixed.push(`回転・渦：${fgSwirlVals.map(v => fgl("swirlType", v)).join("・")}（複数を重ねる）`);
    else if (fg.swirlType !== "auto" && fg.swirlType !== "skip") fixed.push(`回転・渦：${fgl("swirlType", fg.swirlType)}（固定）`);
    else if (fg.swirlType === "auto") vary.push("回転・渦");
    const fgDigitalVals = getMultiVals(details.multiOverrides, "foreground.digitalType");
    if (fgDigitalVals.length >= 2) fixed.push(`HUD・デジタル：${fgDigitalVals.map(v => fgl("digitalType", v)).join("・")}（複数を重ねる）`);
    else if (fg.digitalType !== "auto" && fg.digitalType !== "skip") fixed.push(`HUD・デジタル：${fgl("digitalType", fg.digitalType)}（固定）`);
    else if (fg.digitalType === "auto") vary.push("HUD・デジタル");
    const fgArtVals = getMultiVals(details.multiOverrides, "foreground.artType");
    if (fgArtVals.length >= 2) fixed.push(`アート表現：${fgArtVals.map(v => fgl("artType", v)).join("・")}（複数を重ねる）`);
    else if (fg.artType !== "auto" && fg.artType !== "skip") fixed.push(`アート表現：${fgl("artType", fg.artType)}（固定）`);
    else if (fg.artType === "auto") vary.push("アート表現");
    const fgPosVals = getMultiVals(details.multiOverrides, "foreground.position");
    if (fgPosVals.length >= 2) fixed.push(`位置：${fgPosVals.map(v => fgl("position", v)).join("・")}（複数箇所に配置）`);
    else if (fg.position !== "auto" && fg.position !== "skip") fixed.push(`位置：${fgl("position", fg.position)}（固定）`);
    ...
    const fgMotionVals = getMultiVals(details.multiOverrides, "foreground.motion");
    if (fgMotionVals.length >= 2) fixed.push(`動き：${fgMotionVals.map(v => fgl("motion", v)).join("・")}（複数の動きを併用）`);
    else if (fg.motion !== "auto" && fg.motion !== "skip") fixed.push(`動き：${fgl("motion", fg.motion)}（固定）`);
    else if (fg.motion === "auto") vary.push("動き");
    const fgColorVals = getMultiVals(details.multiOverrides, "foreground.color");
    if (fgColorVals.length >= 2) fixed.push(`色方向：${fgColorVals.map(v => fgl("color", v)).join("×")}（複数色を併用）`);
    else if (fg.color !== "auto" && fg.color !== "skip") fixed.push(`色方向：${fgl("color", fg.color)}（固定）`);
  ...
  if (scopes.includes("pose")) {
    ...
    const poImpVals = getMultiVals(details.multiOverrides, "pose.impression");
    if (poImpVals.length >= 2) fixed.push(`印象：${poImpVals.map(v => (POSE_LABELS.impression as Record<string, string>)[v] ?? v).join("・")}（複数の印象を融合）`);
    else if (p.impression !== "auto" && p.impression !== "skip") fixed.push(`印象：${POSE_LABELS.impression[p.impression]}（固定）`);
    else if (p.impression === "auto") vary.push("印象");
    const poHandVals = getMultiVals(details.multiOverrides, "pose.hand");
    if (poHandVals.length >= 2) fixed.push(`手：${poHandVals.map(v => (POSE_LABELS.hand as Record<string, string>)[v] ?? v).join("・")}（左右の手で異なる配置）`);
    else if (p.hand !== "auto" && p.hand !== "skip") fixed.push(`手：${POSE_LABELS.hand[p.hand]}（固定）`);
    else if (p.hand === "auto") vary.push("手の位置");
    const poFootVals = getMultiVals(details.multiOverrides, "pose.foot");
    if (poFootVals.length >= 2) fixed.push(`足：${poFootVals.map(v => (POSE_LABELS.foot as Record<string, string>)[v] ?? v).join("・")}（左右の足で異なる配置）`);
    else if (p.foot !== "auto" && p.foot !== "skip") fixed.push(`足：${POSE_LABELS.foot[p.foot]}（固定）`);
    ...
    const poMotionVals = getMultiVals(details.multiOverrides, "pose.motion");
    if (poMotionVals.length >= 2) fixed.push(`動き：${poMotionVals.map(v => (POSE_LABELS.motion as Record<string, string>)[v] ?? v).join("・")}（複数の動きを併用）`);
    else if (p.motion !== "auto" && p.motion !== "skip") fixed.push(`動き：${POSE_LABELS.motion[p.motion]}（固定）`);
...
function bodyFixBlock(): string {
  return [
    ...
    "・体型比率ガード：頭身・体型のプロポーションを元画像と同じに保ち、脚や胴を縦に引き伸ばさない。魚眼・広角・煽り・俯瞰などの歪み系アングルは使わない。標準〜ポートレートレンズを使う。",
    "─ 出力時の文例（そのまま使ってよい）─",
    "【人体補正】手は片手5本指・自然な関節構造・手首肘なめらか。脚・膝・足首・接地感を物理的に正確に整合。体型のプロポーションを元画像と同じに保ち、脚や胴を縦に引き伸ばさない（標準〜ポートレートレンズ使用・広角歪み・魚眼不使用）。" +
      " Anatomically correct hands (5 fingers each), wrists, elbows, legs, knees, ankles, grounding. Keep body proportions matched to the reference, do not elongate legs or torso; use standard-to-portrait lens only, do not apply fisheye or wide-angle distortion.",
...
function viralBlock(pt: PromptTarget = "full"): string {
  const lines = [
    ...
    "- 意外な世界観の組み合わせを積極採用：上品×ストリート / 広告×インダストリアル / Y3K×和紙 / ゴシック×ポップ / 工場跡×高級ブランド",
 ...
 * 背景の風景・空間だけを 2D/イラスト調へ寄せる。人物（顔・肌・髪・衣装・体型）は元画像の実写質感を厳守し、絶対にアニメ化しない。
 * ※前回のリグレッション（人物までアニメ化）を防ぐため、人物の実写維持を本ブロック内で最優先明記する。
 */
function avoidRealBackgroundBlock(): string {
  return [
    "【背景を2D/イラスト調に — 背景が変更対象の時のみ・人物は実写厳守】",
    "背景の風景・空間は2D〜イラスト調で描く：アニメ背景／コンセプトアート／絵画的背景／スタイライズされた情景。",
    "実写写真のような背景（住宅街・普通の路地・生活感のある建物・現実スナップ風の街角）は強く避ける。",
    "推奨: 2D illustrated background, anime-style background, concept art environment, painterly backdrop, stylized 2.5D space, designed fashion editorial set",
    "回避: photorealistic background, realistic residential street, documentary photo background, raw camera photo background, mundane city snapshot",
    "",
    "★最重要（絶対厳守）：人物（顔・肌・髪・衣装・体型）は元画像の実写質感をそのまま維持し、絶対にアニメ化・2D化・イラスト化しない。",
    "　仕上がりは「実写の人物 × イラスト調の背景」という意図的な質感差の合成にする。背景だけを様式化し、人物は写真品質のまま立たせる。",
    "※背景が人物を食わないよう、被写体を主役に立てる。",
  ].join("\n");
}

// ※ stylizedBackgroundBlock は撤去（6/3基準の実写維持へ復元）。
//   背景の2D/スタイライズ/アニメ寄せ（"anime-inspired environment" 等）は元画像の実写質感を壊す主因のため削除した。
//   背景モード（realismLevel）でスタイライズしたい場合は realismBlock 側の Lv<=2 指定で表現する。
...
function avoidClicheBlock(): string {
  return [
    ...
    "",
    "▼ ズラし方の原則（具体的な場所・モチーフは各案で自由に発想し、毎回違うものにする）：",
    "  - 定番の場所・小物・花・色から離れ、予想外の組み合わせを毎回ひねり出す。",
    "  - 同じ「ズラし先」を案またぎ・生成またぎで繰り返さない（温室・遊園地・廃墟ばかりにしない）。",
    "  - 衣装の系統はそのままに、舞台・小物・光だけを意外な方向へ振る。",
    "  - 花・小物・背景は特定の定番（バラ・桜・羽・HUD等）に寄せず、毎回ジャンルごとに異なる発想で選ぶ。",
    "",
...
function outfitDiversityBlock(): string {
  return [
    ...
    "  ✓ ゴシック / ゴシックドール系 / ゴシックモード が指定されている",
...
/**
 * ビジュアル設計の基本哲学ブロック。
 * このアプリが目指すのは視覚的インパクト・意外性のある組み合わせ・
 * 印象に残るビジュアル完成度の追求。
 */
function userStyleBlock(): string {
  return [
    "【ビジュアル設計の基本哲学】",
    "このアプリの目的は、視覚的インパクト・意外性のある組み合わせ・印象に残るビジュアル完成度を追求すること。",
    "「すべての要素が画として強く機能するなら積極的に使う。避けるのは意味なく詰め込むだけの構成。」",
    "",
    "▼ 構図・視認性（常時）：",
...
function bgDiversityBlock(req: GenerateRequest): string {
  ...
  return [
    ...
    "  高級スタジオ / 広告的ニュートラル壁 / 影だけの背景 / 無機質な質感の壁面 /",
    "  柔らかい逆光 / ぼけた都市 / 反射床 / 薄い霧 / 淡いグラデーションのある奥行き背景 /",
    "  シンプルな屋上 / モノトーン壁 / 光と陰影のある抽象背景 / 静謐な自然（空・遠景・草原の奥行き）",
    "  いずれも単色・グラデーションだけの平面にせず、光と陰影・奥行きを感じさせる仕上げにする。",
    "",
...
function poseDynamicsBlock(): string {
  return [
    "【ポーズ設計 — 自然な動き・佇まいの良さ】",
    "棒立ち（ただ立っているだけ / 正面を向いて直立するだけ / 正面バストアップ固定）は使わない。",
    "ただし劇的・アクロバティックな瞬間を狙わず、「自然な動き」「重心の移動」「視線・佇まいの良さ」が感じられる落ち着いたポーズを選ぶ。",
    "",
    "▼ 採用すべきポーズの方向性（案ごとに明確に変える）：",
    "  ゆっくり歩く / 歩きながら振り返る / 片脚に重心を預けて立つ / 軽く重心を片脚へ移す /",
    "  腰かける・座る / しゃがんで何かを見る / 振り向く途中の角度 / 肩ごしの視線 /",
    "  片手を髪や襟元に添える / 上半身に角度をつける / 歩を止めて佇む /",
    "  窓辺や壁にもたれて立つ / 服や髪が軽く揺れる程度",
    "※ ポーズは健全でファッション誌的な範囲に保つ（挑発的・媚びる・体の強調は避ける）。",
    "※ 走り出す・飛び降りる・ジャンプ・壁を蹴るなどの劇的アクションは、明示指定がない限り自動では選ばない。",
    "",
    "▼ ポーズを確定する前の確認：",
    "  □ 「ただ立っているだけ」「正面を向いて直立」になっていないか",
    "  □ 逆に劇的すぎる瞬間（走り出す・跳ぶ・蹴る）になっていないか",
    "  □ 動きや佇まいが自然で、無理のない重心になっているか",
    "  □ 各案でポーズが明確に異なるか",
  ].join("\n");
}
...
function weirdCombinationBlock(): string {
  return [
    ...
    "",
    "▼ 推奨する衝突パターン（例）：",
    "  上品×無骨（インダストリアル） / 広告ビジュアル×倉庫街 / Y3K×和紙・墨 / ゴシック×ポップカラー /",
    "  倉庫×高級ブランドファッション / 軍服×花畑 / 工事現場×ランウェイ /",
    "  和装×未来都市 / サイバー×土の庭 / スーツ×工場跡 /",
...
function cameraAggressionBlock(): string {
  return [
    "【カメラ設計 — 落ち着いた自然なアングル採用】",
    "「正面・バストアップ・ふつうの三分割」をデフォルトにしない。",
    "カメラの選択それ自体が“単調でない自然さ”になるアングルを採用する（誇張で歪むアングルは避ける）。",
    "",
    "▼ 採用すべきカメラの方向性（案ごとに明確に変える）：",
    "  目線の高さの斜め45度 / 標準レンズ感の自然な構図 / 目線の高さからの落ち着いた構図 /",
    "  落ち着いたローアングル（あおりすぎない）/ 背後からの柔らかい逆光シルエット /",
    "  顔に寄った接近クローズアップ（誇張広角は使わない）/ 斜め横からの落ち着いた横アングル /",
    "  映画スチール風の斜め後方ショット / アーティスト写真風の横顔 /",
    "  鏡越し・ガラス越し反射 / 画面端に寄せた大胆なトリミング（人物は主役サイズを保つ）/",
    "  歩く・座る・自然に立つ動作を、自然な目線の高さ・距離で捉えた瞬間",
    "",
    "▼ レンズ・人物比率の方針（等身崩れ防止・最優先）：",
    "  レンズは標準〜ポートレートレンズに固定する。魚眼・誇張広角・地面すれすれの超ローアングル・俯瞰（見下ろし）・強い煽りは使わない（体や脚が伸びて歪むため）。カメラ高さは目線の高さ前後を基本にする。",
    "  人物は画面内で主役の大きさを保ち、豆粒化・遠景の点景にしない。アングルの演出より人物の比率（元画像基準の自然な頭身）を優先する。",
    "  Use standard-to-portrait lens ONLY; do not use fisheye, ultra-wide, ground-level low-angle, high-angle (looking down), or strong low angles. Keep the camera near eye level, the subject prominent, and proportions matched to the reference (no leg/torso elongation).",
    "",
    "▼ フレーミング（被写体サイズ維持・最優先）：",
    "  被写体が画面に占める大きさ・写す範囲は元画像を基準に保つ。アングルや構図は案ごとに変えてよいが、指定がない限り元画像より引いて全身にしない（バストアップ〜膝上の元画像をフルボディに広げない）。",
    "  Keep the subject's on-screen size and crop matched to the reference; vary the angle, but do NOT zoom out to a fuller or full-body shot unless explicitly specified.",
    "",
    "▼ カメラを確定する前の確認：",
    "  □ 「正面バストアップ」「正面全身」がデフォルトになっていないか",
    "  □ このカメラアングルが“単調でない自然さ”を持っているか",
    "  □ 各案でカメラアングルが明確に異なるか",
    "  □ 魚眼・誇張広角・超ローアングルや煽り/俯瞰（見下ろし）になっていないか",
    "  □ 元画像より引いて全身になっていないか（被写体の大きさが元画像と同程度か）",
    "  □ アングルの誇張で頭身・脚の長さ・顔の大きさが崩れていないか",
  ].join("\n");
}
...
function safetyOutputGuideBlock(): string {
  return [
    ...
    "  ただし上品さ・モード感は無彩色を意味しない。彩度のある色も上品・洗練になり得るので、色を白・黒・グレーに限定しない。",
    "",
    "▼ ポーズ・印象の記述方針：",
    ...
    "",
    "▼ 自傷・自殺を連想させる語は使わない（ChatGPTの安全フィルタ対策・これらのみ禁止）：",
    "  リストカット・自傷・首吊り・流血・刃物で切る 等の直接的な自傷/流血表現は使わない。",
    "  ※グランジ/ゴシック/インダストリアル等の「質感・空気感」はエディトリアルとして表現してよい。ただし「廃墟」「荒廃」「世紀末」等のliteralな語ではなく、素材感・陰影・無骨さ・空気で表現する。",
    "",
    "▼ 一発バズりモードの表現方針：",
...
export function safetySanitizePrompt(
  ...
  const rules: Array<[RegExp, string]> = [
    ...
    // ── 退廃/グランジ/廃墟系の穏当化は撤去（5月の攻めた語彙へ回帰・統合プラン Part B②）──
    //   廃墟・錆びた・退廃的・朽ちた・破壊的・荒廃 等は「変更を促す」表現としてそのまま通す。
    //   自傷/自殺の直接表現のみ builder ガイドと下段ルールで対処する。

    // ── 英語退廃語の狙い撃ち（ChatGPT画像ブロック対策・タスク#69）───────────────
    //   日本語のグランジ/退廃ゴシック/錆には触れない（味として保護）。
    //   「廃墟」「荒廃」等のliteral語が英語で混入した場合のみ中立化する。
    //   rust（錆び・色名）は味として保護 → 置換しない。
    [/\bdystopian\b/gi,                           "edgy-editorial"],
    [/\bpost-?apocalyptic\b/gi,                   "raw industrial-styled"],
    [/\bapocalyptic\b/gi,                          "dramatic editorial"],
    [/\bwasteland\b/gi,                            "vast minimalist landscape"],
    [/\b(?:decaying|decayed|decay)\b/gi,           "weathered-textured"],
    [/\b(?:ruined|ruins?)\b/gi,                    "raw industrial backdrop"],
    [/\bderelict\b/gi,                             "raw industrial"],
    [/\bdesolate\b/gi,                             "minimalist sparse"],
    [/\b(?:rotting|rotten)\b/gi,                   "weathered"],
    [/\bgore\b|\bgory\b/gi,                        "dramatic editorial"],
    [/\babandoned\s+(?:building|structure|place|site|factory|warehouse)\b/gi,
    ...
    // ── 媚び・くねり系ポーズ（性的表現フィルタ誤判定の穏当化）─────────────────
    [/体をひねりながら/g,                       "上半身に角度をつけて"],
    [/体をひねる/g,                             "上半身に角度をつける"],
    [/体をくねらせ(?:た|る|て)?/g,              "しなやかに動かし"],
    [/くねらせ(?:た|る|て)?/g,                  "しなやかに動かし"],
    [/腰をひねる/g,                             "上半身に角度をつける"],
    [/媚びる(?:ような)?/g,                      "印象的な"],
    [/しなを作る/g,                             "動きをつける"],
    [/前傾姿勢/g,                               "颯爽とした姿勢"],
    ...
    [/生々しい/g,                              "ドラマチックな"],
    // ↓ 官能・セクシー・扇情・魅惑は上段ブロックで処理済み（重複除去）
    [/色気のある/g,                            "存在感のある"],
    [/艶めかし/g,                              "美しく"],
    [/なまめかし/g,                            "魅力的に"],
...
export function buildSystemPrompt(
  ...
  const premiseExample = pt === "nano_safe"
    ...
    : `【前提】この画像はAIで生成された架空キャラクターです。画像編集として${scopeListForExample}のみを変更してください。`;

  const lengthGuide = pt === "nano_safe"
    ? "- 1 案あたり全体で 7〜10 行程度の軽量な長さに収める（Nano Banana 向け）。"
    : "- 1 案あたり全体で 7〜11 行程度のやや短めに収める。各セクションは簡潔に、冗長な装飾語を避ける。";
  ...
  const unifiedFormat = [
    ...
    premiseSectionSpec,
    fixedSectionSpec,
    "- 【変更】具体的な変更内容。素材・色・形・場所・アングル・小物名など具体語で 1〜2 行（簡潔に）。",
    "- 【雰囲気】mood の質感・トーン・色味を 1 行。",
    "- 【光】光源方向・色温度・影・空気感を 1 行。",
    "- 【品質】高解像度／自然な影と反射／違和感のない合成感 を 1 行（簡潔に）。",
  ...
  return [
    ...
    // リアル背景回避：背景が変更対象 かつ トグルON の時だけ注入（既定OFFへ変更。トグルで任意ON可能）
    (req.scopes.includes("background") && req.avoidRealBackground === true) ? "\n" + avoidRealBackgroundBlock() : "",
    // 背景スタイライズ注入は撤去（6/3基準の実写維持へ復元）
