...
- **S1b（S1の追補・commit `f994426`）**: S1の静的シルエット(頭＋胴カプセル)はスクショ確認で「縦棒」化＋回転対称となり**正面/背面/横が判別不能**（yawをドラッグしても見た目が変わらず「視覚で角度を決める」目的を満たさない）と判明。→ ユーザー承認のうえ**向き判別できる簡易人型**へ作り直し（頭＋**シアンの顔向きマーカー**(+Z前面)＋肩＋胴＝横広×前後薄＋固定の腕/脚＋接地リング／**関節なし・固定形状＝ポーズ機能ではない**）。`TARGET` を y=3.6→4.1 に上げ頭〜胴をフレーム内に収める。3Dビューの表示のみ・出力/payload/サーバ/§5 不変。検証＝front tsc 0・build 0・Playwright隔離4330で人型描画(頭/顔マーカー/肩/胴/腕/脚)・ドラッグで顔マーカーが向きに追従・head可視・console エラー0/警告0。
- **S1c（レイアウトのコンパクト化・commit `0f2f2fc`）**: 3Dピッカーを縦積み（横長Canvas＋下に出力文）から**2カラム**（左＝3:4縦長Canvas 150×200 / 右＝出力文）へ。出力文フォントを text-xs→**text-sm(14px)** に拡大、余白を詰めて全体高さを圧縮。OrbitControls操作・`describeCameraAngle`出力・人型・payload・サーバ・CameraContent は不変（見た目のみ）。検証＝front tsc 0・build 0・Playwright隔離4330で Canvas比率0.75(3:4)・出力文が右カラム(14px)・ドラッグで出力追従・console エラー0/警告0。
- **S1d（人型マネキン化＋白背景＋拡大・commit `9430195`）**: 箱組み人型を**滑らかな簡易マネキン**へ（`LatheGeometry` の回転体胴を前後に薄く＝女性寄りテーパー＋向き判別／`CapsuleGeometry` の四肢／頭＋目＋鼻）。**外部3Dモデルは不採用**（参考画像級＝彫刻メッシュはプリミティブの上限外と提案・ユーザー承認のうえ B案=ジオメトリ改良で着手）。**背景を白**（図形はニュートラルグレー＋ソフトライティング）。**人型を2倍以上に拡大**：Canvas 150×200→**264×352(3:4)**、fov 30→46・`TARGET` 4.1→4.7 で頭〜腿を大きく。**初期カメラ距離は据え置き**＝`compositionFromDistance`/`describeCameraAngle` 出力は不変（不変条件の両立）。右の出力文カラムは max-w で必要十分に縮小（左ビュー優先）。S/M/L切替は付けず初期「大きめ」固定。OrbitControls操作・payload・サーバ(§4)・CameraContent・ポーズ非復活は不変。検証＝front tsc 0・build 0・Playwright隔離4330で 白背景・人型拡大(頭/目/鼻/肩/腕/胴/腿)・ドラッグで出力追従(横顔/ローアングル)・console エラー0/警告0。

### 2026-06-15: Reference Picker 貼付画像が生成用画像欄に混入するバグ修正（④単独・commit `ff61f6f`）

- **症状/原因**: `ImageUploader`(生成用) と `ReferenceImportPanel`(ピッカー) が**両方 `window` の 'paste' を購読**し、1回の Ctrl+V 画像貼付で両ハンドラが発火→main(`imageDataUrl`)とpicker(`image`)へ**二重ロード**（左の生成用画像選択欄への混入）。state は別物だが paste 経路が共有・ピッカーは折りたたみ中も購読・ガード非対称（ImageUploader は入力欄フォーカス時スキップ／ピッカーは無条件）。調査で `referenceNote`→extraInstructions の適用経路や onContextChange は混入に無関係と確認（混入は paste 限定・DnD/ファイル選択は要素単位で隔離）。
- **修正（`ReferenceImportPanel.tsx` のみ）**: paste 購読を **open の時だけ**に限定（`if(!open) return` ＋ deps に open・閉=購読せず生成用へ渡す）＋ **capture フェーズ**(`addEventListener("paste",fn,true)`)で ImageUploader(bubble)より先取り＋画像処理時に `e.stopImmediatePropagation()` で他リスナーへの伝播停止。結果＝**ピッカーopen中の貼付=ピッカーのみ／ピッカー閉=生成用のみ**。テキスト貼付・DnD・ファイル選択は不変。**ImageUploader は無変更**。
...
- **B 右クリック貼付（実装済・commit `d829a2e`）**: 実現可否を先に提案（**Chrome/Edge は可**・Firefox/Safari/権限拒否は不安定）→ユーザー承認のうえ**フォールバック前提で実装**。画像 dropzone に `onContextMenu`→標準メニュー抑制→独自メニュー「📋 クリップボードから貼り付け」→`navigator.clipboard.read()` で image/* を `loadFile`（メニュークリック＝ユーザー操作起点で gesture を満たす）。**フォールバック**＝未対応/権限拒否/画像なし/失敗→「Ctrl+V で貼り付けてください」トースト（Ctrl+V は常に有効＝二重化）。`preventDefault` は画像小領域のみ・外側クリックでメニュー閉。**第1便の paste 分離（open限定＋capture＋stopImmediatePropagation）は無変更の別経路**＝Ctrl+V 健在。ImageUploader/referenceNote→extraInstructions/payload/サーバ/§4 も無変更。検証=front tsc 0・build 0・Playwright隔離4330（clipboard権限付与）で 右クリック→メニュー→`clipboard.read`→ピッカー読込・**左欄混入なし**・メニュー閉、Ctrl+V併存も読込＆混入なし、console エラー0/警告0。Playwright知見=clipboard.write は headless で 1x1 PNG が DataError になり得るが read 経路は正常。

### 2026-06-15: Reference Picker履歴（旧 Compare Mode を土台に「履歴/アルバム」へ育成・A〜E 全完了）

- **背景/方針**: 参照画像＋抽出プロンプトの「履歴/アルバム」を、既存 Compare Mode（参照↔生成比較）を**土台に育てる**方針。調査①〜④で保存構造を確認（`referenceRecords` ストア・idb v7・index batchId/createdAt／保存トリガは「生成時＋適用1軸以上」のみ＝ピッカー単独保存の経路が無い＝アルバム化の本当の穴は**保存トリガ**）。設計 A〜E をユーザー承認（**育てる案**＝referenceRecords を任意フィールドで拡張・新規ストアは作らない／D保存＝明示「📌 履歴に保存」ボタン／画像＝**サムネ(refThumb)のまま**）。**§4(idb.ts)は不可触＝ReferenceRecord は schemaless オブジェクトのため任意フィールド追加に migration 不要**。PC専用。1段ずつ実装→検証→各段コミット（push は別途指示）。
- **A 改名＋C 全画面（S1・commit `f0c2a3b`）**: `CompareModeView` を「Compare Mode」→**「Reference Picker履歴」**へ改名（ヘッダ/サブタイトル/ピッカー入口「🆚 比較」→「🕘 履歴」・`SkyveilBar`「🆚Compare評価」→「🆚参照比較の評価」）。中央モーダル(max-w-[1400px]/h-[92vh]/backdrop)を**全画面**(`fixed inset-0`)化＋body overflow ロック。比較/評価ロジックは不変。
- **B 左メニュー入口（S2・commit `5760f68`）**: Explorer(`ImageSidebar`) の カレンダー/お気に入り の並びに「🖼 Reference Picker履歴」ボタンを新設（`onShowReferenceHistory`→App `setCompareOpen(true)`）。ピッカーを開かずに履歴へ入れる第2導線。
- **E 復旧ボタン移設（S3・commit `103d47c`）**: 「🛟 履歴・お気に入り復旧」を**左メニューから撤去**し、お気に入り一覧/履歴画面(`HistoryView`)の**左レール最下部**へ移設（誤操作防止）。App で 🛟 ボタン＋`RecoveryPanel` を構築し `recoverySlot` ReactNode prop で `HistoryView` の `<aside>` 内へ注入（当初の固定オーバーレイ案は左レール既存内容と重なり→slot方式へ修正）。`RecoveryPanel`/`recoveryOpen`/`reloadAllData` は無変更＝**機能は移動のみ**。`HistoryView` 既存「データ管理」(export/import/cleanup)の直下に併存。
- **D 保存(📌)（S4・commit `83c409c`）**: `referenceRecords.ts` に任意フィールド `kind?("generation"|"picker")`/`note?` 追加（後方互換＝未設定は generation 扱い・idb 不変）。`ReferenceImportPanel` に「📌 この参照を履歴に保存」ボタン（`onSaveToHistory` prop・画像必須）。App `handleSaveReferenceToHistory` は**既存の生成時保存と同じデータ源**（`referenceContextRef`＝画像+抽出 / `referenceNote`＝適用）で `kind:"picker"`/`batchId:""` 保存＝**生成しなくても履歴に残せる**。`CompareModeView` 一覧に「📌保存」バッジ＋空状態コピー微修正（ピッカー保存レコードは batchId 空→結果は「未登録」へ degrade・クラッシュ無し）。
- **D 再利用(♻)（S5・commit `e24d54d`）**: `CompareModeView` 詳細ペインに「♻ この参照をピッカーで再利用」（`onReuse` prop・一覧項目は `<button>` のためネスト回避＝選択中を詳細から再利用）。App `handleReuseReference`＝レコードの refThumb＋extracted を **seed 化**し履歴を閉じ `setView("main")`・**token で同一レコードの再利用も再発火**。`ReferenceImportPanel` に `reuseSeed`/`onReuseConsumed` prop＋effect で画像＋13カテゴリを**全置換**で流し込み open。**消費後に seed を null へ戻し**（`onReuseConsumed`）、ビュー切替でピッカーが unmount→再mount しても**二重注入しない**。再利用後の「適用」はユーザーが従来どおり押す＝**適用ロジック不変**。
- **不変条件（全段共通・実証済）**: 抽出(`referenceExtract`)/適用(referenceNote→extraInstructions)/生成ロジック・`/api/generate` ペイロード・サーバ(§4 promptSystem/gemini/scopeFilter)・idb.ts(§4・DB_VERSION/store/upgrade)・既存の生成時自動保存 は無変更。新規「見えない支配」設定なし（§5：履歴は全案に効くグローバル設定ではない）。
- **検証（各段 §6・Playwright隔離4330・空DB）**: 全段 front tsc 0・vite build 0。S1=全画面化＋改名描画。S2=左メニュー入口→履歴 open。S3=main 左メニューに復旧0件／お気に入り一覧 aside 内に復旧(static)＋開いて RecoveryPanel・重なり無し。S4=画像注入→📌保存→IDB `kind:"picker"`/`batchId:""`/サムネ有/`extracted` 捕捉→🕘履歴で 📌保存バッジ・抽出が比較表・結果「未登録」degrade。S5=seed→🕘履歴→♻→画像＋背景/衣装注入・Compare閉・flash／二度目=token 再発火で全置換／ビュー再mountで再注入なし。全段 console エラー0/警告0・後片付け済(4330のみ停止・5173/3001不可侵)。
- **残課題（別途）**: A=抽出品質の実Gemini質的確認／アルバムのラベル(`note`)入力UI（フィールドは予約済・未配線）／参照レコード一覧サムネの拡大・整理(削除/お気に入り)UI 拡充。

### 2026-06-15: UI整理4点（カレンダー入口移設／簡易編集系2ボタン削除／生成プログレス上部移動・拡大／接続バッジ復活）

- **背景**: ユーザー要望のUI整理4点（表示・配線のみ・段階実装）。各段 front tsc/vite build＋最終 Playwright隔離4330 で一括検証。push 済（`d899883..a8a3a52`）。
- **S1 カレンダー入口を左メニューへ（commit `6aa59d0`）**: 右上(中央パネル上端)の「📅 1ヶ月生成カレンダー」誘導ボタンを撤去し、`ImageSidebar` の お気に入り一覧／Reference Picker履歴／分析センター の並び(分析センター直後)へ移設。新 prop `onShowPostingCalendar`＝App から `setAnalysisInitialTab("plan")＋setAnalysisCenterOpen(true)`（**遷移先=分析センター plan タブは不変・配置のみ**）。左メニュー既存「📅 カレンダー/履歴」(履歴画面)は別物で不変。
- **S2 簡易画像編集/選択範囲プロンプト削除＋デッドコード掃除（commit `d297deb`）**: 両ボタン(画像セット時表示)を撤去。`SimpleImageEditor.tsx`/`SelectionPromptModal.tsx` は App からのみ参照＝孤立につき**ファイル削除**(−1625行)。App の import/state(selectionModalOpen/simpleEditorOpen)/配線/モーダル描画、ImageSidebar の props(onOpenSelectionPrompt/onOpenSimpleEditor)＋ボタンを撤去。`useEscapeKey.ts`/ImageSidebar の stale comment(削除コンポーネント名)を一般化。**永続層は据え置き**＝`selectionHistory.ts`/idb `selectionHistory` ストア/`backup.ts` は §4 かつ backup が現に利用([backup.ts:26-27,189])・既存データ保全のため不変(UI からの書込が止まるのみ)。
- **S3 生成プログレスを上部バーへ移動＋拡大（commit `afc0259`）**: `GenerationProgress` に `size?("sm"|"lg")` 追加(lg=バー高 h-2→h-3.5・ステージ文 text-sm→text-base・スピナー拡大・上罫線+余白でストリップ化／**進捗計算・数値・ステージ語は不変**)。`GlobalProtectionBar` に `progressSlot` スロット新設(本体行=🔎分析センターで見る を含む行 の直下・全幅・表示専用)。App から `<GenerationProgress size="lg">` を progressSlot 配線(onComplete=handleGenerationComplete を移動)。`GenerationSummary` から進捗バー＋generating/onComplete props を撤去(設定サマリ文のみ残す)。
...
### 2026-09-20: 品質調査・軽微修正（未commit / 未push）

- ユーザー指示の範囲で、短縮後処理の文字数増加・複数行NG欠落、結果画像の重複key、狭い画面の評価欄折り返し、旧UI説明を修正。
- front/server型チェック・本番build・短縮回帰テスト6件・隔離4330/4331でのUI確認を実施。実API・5173の保存データは不使用。
- 保留: 固定対象の最終フィルター漏れ、非表示UIの自動学習、解除入口のない旧反映ON設定、タグNG本文合流、背景候補除外の不足。生成・学習・保存仕様の変更は未実施。
- 現在の実装は過去ログの「tagNgは本文非合流」「温室再採用は根治」と一致しない部分がある。再現条件・根拠・承認が必要な判断は [品質調査報告](33_quality-audit-2026-09-20.md) を参照。
