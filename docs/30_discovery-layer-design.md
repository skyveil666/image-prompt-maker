...
## 7. 定型プロンプト除外層（P3a 改善・承認 2026-06-07）
**背景**: 履歴 promptText はサーバ生成の統一プロンプト＝毎回ほぼ同一の保護/品質スキャフォールド（【同一性保護】【顔崩れ防止】【前提】【固定】【品質】＋人体補正/anatomically correct hands）を含むため、定型語が最高頻度になり本来の発見候補を埋もれさせていた。

**ユーザー決定**: 構造除去＋頻度カット＋定型語（3層）／Layer1=可変セクションのみ allowlist／頻度閾値 ≥75%／今すぐ実装。

**実装**（`discoveryMotifs.ts` の前処理強化のみ・検出方式/保存は不変）
- **Layer1 構造ブロック除去**: `extractVariableText()`＝`【…】` を解析し**可変セクションのみ**採用（VARIABLE_SECTION_KEYS＝変更/雰囲気/スタイル/世界観/色/絵柄/演出/前景/小物/カメラ/ライティング/背景/衣装/髪/ポーズ/コスプレ/機械/乗り物/神話/大物/構図/トレンド）。`【】` 無しは全文フォールバック（旧/プレーン互換）。
- **Layer2 定型語**: STOPWORDS に前提/固定/維持/同一性/輪郭/質感/アスペクト比/品質/高解像度/人体補正/手/片手/5本指/指/保護/防止/架空/キャラクター/anatomically/correct/hands/preserve/maintain/identity/face… を追加。
- **Layer3 文書頻度カットオフ**: 全件の **75%超**に出る語を除外（`count/total ≥ 0.75`）。`total < 10` 件のときは無効化（薄い履歴の誤除外回避）。

**検証**（tsc -b ✅ / vite build ✅ / Playwright隔離・定型セクション付き現実的プロンプト10件投入）
- 候補から **定型語ゼロ**（前提/固定/品質/高解像度/人体補正/手/指/anatomically/correct/hands すべて除外）を確認。
- 【変更】内の新語（glorptone palette）と wibblecore aesthetic は**残存**。コンソール0エラー。

**既知の限界**: 日本語の自由文は助詞付きの塊で出る（例「を取り入れた新様式」＝形態素解析非採用のため）。区切り（、・/）で書かれた語・英語様式名は綺麗に出る。実運用で要観察→必要なら助詞分割や監視済み区間除外を将来検討。

**End of Doc 30（P3a＋定型除外層 実装・検証済み・未push／P3b 未着手）**
