...
// ═══════════════════════════════════════════════════════════════════════════════
// 🌌×🖌 斬新背景×画法世界の融合ブリッジ（commit3・2枠50:50ミックス）
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * 🌌×🖌 斬新背景×画法世界の融合ブリッジ文（commit3・buildWorldBgBridgeNote と同型・front-only・§4不触）。
 * 斬新背景プリセットと画法世界プリセットが「それぞれ1つずつ」かつ背景が変更対象のときだけ、
 * App 側で buildInputs の extra へ（bgPresetNote/artPresetNote に加えて）挿入する。
 * 世界観×斬新背景のブリッジは総称的な「プリセット」表現だが、こちらは2つとも固有の質感を持つ
 * 具体プリセットのため、抽象語だけでは融合の方向性が伝わりにくいと判断し、両プリセット名を
 * 明示的に埋め込む形にした（世界観×斬新背景とは違い引数を取る）。
 * details.background の WIRED フィールドは App.tsx 側の改修で「相手カテゴリが非0件の時は
 * setDetails しない」＝両方選ばれている間は一方の値に固定しない。融合の表現はこのテキストが担う。
 */
export function buildBgArtBridgeNote(bgPreset: BgPreset, artPreset: ArtPreset): string {
  const bgLabel  = BG_PRESET_DISPLAY[bgPreset];
  const artLabel = ART_PRESET_DISPLAY[artPreset];
  return [
    `【${bgLabel} × ${artLabel} の融合指示】`,
    `「${bgLabel}」と「${artLabel}」を同時に指定している。背景はどちらか一方に寄せず、両者の要素を約半々で混ぜ合わせ、1つの画面に共存させること。`,
    "各案で「一方の背景だけ」へ振り分けず、毎案で両方の質感・世界観が溶け合った融合背景にする。",
    "▼ 奥行き：融合した背景でも、前景から奥へ層を成す遠近感を保つこと（どちらか一方の奥行き表現だけに寄らず、両者の質感が前後の層として折り重なる形にする）。",
    "色は特定の色を固定せず、両プリセットそれぞれの持ち味（質感・色数の傾向）を活かした配色にする。",
    "人物・顔・同一性・衣装・構図は上記の指示のまま維持する（融合は背景にのみ適用）。",
  ].join("\n");
}
