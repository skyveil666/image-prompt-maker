...
export type SnsType =
  | "x_buzz" | "instagram" | "tiktok" | "pinterest"
  | "thumbnail" | "icon" | "scroll_stop" | "save" | "double_take" | "global";

interface SnsDirection { type: SnsType; label: string; moods: Mood[]; scopes: Scope[]; hint: string; }

const SNS_DIRECTIONS: readonly SnsDirection[] = [
  { type: "x_buzz",      label: "Xバズ",         moods: ["x_buzz", "vivid", "cinematic"] as Mood[],     scopes: ["background", "camera", "foreground"] as Scope[],  hint: "X（Twitter）でRTされる。高コントラスト・意外性のある構図・一瞬で目を引くビジュアルインパクト。テキストを貼りやすい余白構図も考慮。" },
  { type: "instagram",   label: "Instagram映え",  moods: ["instagram", "portrait", "luxe"] as Mood[],    scopes: ["background", "lighting", "outfit"] as Scope[],    hint: "Instagramのフィード映え。美しい色調・洗練されたライフスタイル感・保存したくなる完成度。フィード全体に合う統一感のあるトーン。" },
  { type: "tiktok",      label: "TikTok映え",     moods: ["tiktok", "sns_pop", "vivid"] as Mood[],       scopes: ["outfit", "foreground", "pose"] as Scope[],        hint: "TikTokで止まる演出。縦長・トレンディ・エネルギッシュ・若者文化の空気感。前景の演出で映像的な動きを予感させる。" },
  { type: "pinterest",   label: "Pinterest映え",  moods: ["pinterest", "pastel", "art"] as Mood[],       scopes: ["background", "lighting", "foreground"] as Scope[], hint: "Pinterestのムードボード的な美しさ。ライフスタイル・インスピレーション・美的探求。上質でキュレーションされた雰囲気。" },
  { type: "thumbnail",   label: "サムネ最強",     moods: ["vivid", "cinematic", "sns_pop"] as Mood[],    scopes: ["camera", "lighting", "background"] as Scope[],    hint: "YouTube・動画サムネイルで最高のクリック率を出す画面。顔・驚き・対比・鮮やかな色・縮小しても目を引く構図。" },
  { type: "icon",        label: "アイコン映え",   moods: ["portrait", "bright", "sns_pop"] as Mood[],    scopes: ["camera", "lighting", "background"] as Scope[],    hint: "SNSアイコンとして完璧な一枚。円形クロップ前提。顔中心・シンプルな背景・清潔感・個性が伝わる表情。" },
  { type: "scroll_stop", label: "スクロール停止", moods: ["vivid", "art", "fantasy"] as Mood[],          scopes: ["foreground", "camera", "background"] as Scope[],  hint: "フィードをスクロールする指が思わず止まる画面。予想外の要素・意外な色の組み合わせ・思わず二度見する視覚的引っ掛かり。" },
  { type: "save",        label: "保存される系",   moods: ["art", "cinematic", "fantasy"] as Mood[],      scopes: ["background", "lighting", "foreground"] as Scope[], hint: "スマホの壁紙にしたくなる・永久保存したくなる質の高い一枚。高解像度感・詩的な美しさ・見るたびに発見がある奥行き。" },
  { type: "double_take", label: "二度見する構図", moods: ["cinematic", "art", "digital"] as Mood[],      scopes: ["camera", "foreground", "background"] as Scope[],  hint: "一瞬で意味がわからない・見直すと発見がある構図。視点のトリック・スケールの誤魔化し・予想を裏切るフレーミングで二度見を誘発。" },
  { type: "global",      label: "海外ウケ",        moods: ["luxe", "cinematic", "instagram"] as Mood[],   scopes: ["background", "outfit", "lighting"] as Scope[],    hint: "海外SNSで受ける国際的な美意識。普遍的な美しさ・高品質感・インターナショナルな雰囲気を優先する。" },
];

export function getSnsLabel(type: SnsType): string {
  return SNS_DIRECTIONS.find((d) => d.type === type)?.label ?? type;
}

export function buildSnsInputs(current: PromptInputs, memory: VariationMemory = createEmptyMemory(), type: SnsType): PromptInputs {
  const dir = SNS_DIRECTIONS.find((d) => d.type === type)!;
  const note = [
    `【📈 SNSバズ — ${dir.label}】`, dir.hint, "",
    "▼ SNSバズの方針：",
    "  - 選ばれたプラットフォームで「止まる・保存される・シェアされる」画面を作る",
    "  - 各案でバズりポイントの切り口（色・構図・要素）を変えて差別化する",
    "  - 過剰なエフェクトより「説得力のある美しさ・意外性・完成度」を優先する",
    "【絶対維持】顔・表情・人物の同一性・体型は一切変更しない。",
  ].join("\n");
  void memory; // 将来の多様性制御のために引数を確保（現在は未使用）
  return { ...current, scopes: scopeOrDefault(current, dir.scopes), moods: dir.moods, faceLock: true, viralMode: false, autoMoodCategories: [], extraInstructions: note, details: DEFAULT_DETAILS, count: 4 as Count };
}

// ─── 🌐 カルチャー ────────────────────────────────────────────────────────────────

export type CultureType =
  | "korea_ad" | "china_future" | "hk_neon" | "shibuya_night" | "harajuku_pop"
  | "tokyo_cyber" | "taiwan_neon"
  | "la_90s" | "nyc_street" | "paris_luxury" | "milan_runway"
  | "berlin_industrial" | "nordic_minimal" | "london_punk"
  | "dubai_luxury" | "mumbai" | "seoul_night" | "osaka_town" | "bangkok" | "sao_paulo";

interface CultureDirection { type: CultureType; label: string; moods: Mood[]; bgPlace: BackgroundPlace; outfitStyle: OutfitStyle; hint: string; }

const CULTURE_DIRECTIONS: readonly CultureDirection[] = [
  // ── アジア ──────────────────────────────────────────────────────────────────────
  { type: "korea_ad",          label: "韓国広告",      moods: ["instagram", "minimal", "stylish"] as Mood[],       bgPlace: "studio",        outfitStyle: "mode",         hint: "韓国高級広告の美学。クリーンな背景・K-beautyの完璧な肌・ミニマルで洗練されたスタイリング・韓国ブランドの広告感。" },
  { type: "china_future",      label: "中国未来広告",  moods: ["near_future", "vivid", "luxe"] as Mood[],          bgPlace: "futuristic",    outfitStyle: "future_dress", hint: "中国の次世代広告美学。赤と金×未来感・大胆なビジュアルインパクト・中華ネオフューチャリズム・スペクタクルな演出。" },
  { type: "hk_neon",           label: "香港ネオン",    moods: ["cinematic", "retro", "dark"] as Mood[],            bgPlace: "alley",         outfitStyle: "street",       hint: "香港夜のネオン街。看板の光・雨濡れの路地・ノワール的な色彩・多文化が混じり合うアジア的混沌と美。" },
  { type: "shibuya_night",     label: "渋谷夜",        moods: ["x_buzz", "cinematic", "vivid"] as Mood[],          bgPlace: "indoor",        outfitStyle: "street",       hint: "渋谷の夜の喧騒。ビル広告の光・現代日本の都市エネルギー・圧倒的な情報量の美。" },
  { type: "harajuku_pop",      label: "原宿ポップ",    moods: ["pop", "vivid", "sns_pop"] as Mood[],               bgPlace: "gallery",       outfitStyle: "y2k",          hint: "原宿スタイルのポップカルチャー。カラフル・個性的・原色×パステル・日本のハイパーポップ文化の爆発。" },
  { type: "tokyo_cyber",       label: "Tokyo cyber",   moods: ["near_future", "cool", "digital"] as Mood[],        bgPlace: "futuristic",    outfitStyle: "cyber",        hint: "東京サイバー（過剰なブレードランナーは禁止）。テック感・研ぎ澄まされたモダニズム・日本的清潔サイバー。" },
  { type: "taiwan_neon",       label: "台湾ネオン",    moods: ["retro", "vivid", "cinematic"] as Mood[],           bgPlace: "indoor",        outfitStyle: "street",       hint: "台湾の夜市・ネオン看板・屋台の光・親しみやすい暖かさと現代感が混じったアジアの原風景。" },
  // ── 欧米 ──────────────────────────────────────────────────────────────────────
  { type: "la_90s",            label: "90s LA",        moods: ["retro", "bright", "street"] as Mood[],             bgPlace: "rooftop",       outfitStyle: "street",       hint: "1990年代ロサンゼルス。プールサイド・ヤシの木・カリフォルニアの太陽・ヒップホップ×スポーツウェアの文化。" },
  { type: "nyc_street",        label: "NYC street",    moods: ["cool", "street", "cinematic"] as Mood[],           bgPlace: "alley",         outfitStyle: "street",       hint: "ニューヨークのストリートスタイル。レンガの壁・マンハッタンの空気感・多様性とファッションの聖地。" },
  { type: "paris_luxury",      label: "Paris luxury",  moods: ["luxe", "art", "cinematic"] as Mood[],              bgPlace: "gallery",       outfitStyle: "runway",       hint: "パリの高級ファッション。フランス的エレガンス・couture文化の本場・オスマン様式建築。" },
  { type: "milan_runway",      label: "Milan runway",  moods: ["luxe", "stylish", "cinematic"] as Mood[],          bgPlace: "atelier",       outfitStyle: "runway",       hint: "ミラノコレクションのランウェイ。建築的シルエット・イタリアの素材感・alta modaの圧倒的な完成度。" },
  { type: "berlin_industrial", label: "Berlin",        moods: ["dark", "art", "emo"] as Mood[],                    bgPlace: "industrial",    outfitStyle: "techwear",     hint: "ベルリンの廃工場・テクノカルチャー・ポストインダストリアル美学・コンクリートと自由の都市。" },
  { type: "nordic_minimal",    label: "Nordic minimal",moods: ["minimal", "cool", "translucent"] as Mood[],        bgPlace: "frosted_room",  outfitStyle: "mode",         hint: "北欧ミニマリズム。白・グレー・木の質感・自然光・機能美・ラグジュアリーな静けさ。" },
  { type: "london_punk",       label: "London punk",   moods: ["street", "dark", "vivid"] as Mood[],              bgPlace: "alley",         outfitStyle: "gothic",       hint: "ロンドンパンク・ポストパンク。スタッズ・プラットフォームブーツ・反骨精神・カオスの中の美学。" },
  // ── 中東・その他 ─────────────────────────────────────────────────────────────
  { type: "dubai_luxury",      label: "Dubai luxury",  moods: ["luxe", "vivid", "cinematic"] as Mood[],            bgPlace: "indoor",        outfitStyle: "runway",       hint: "ドバイの超高級ライフスタイル。黄金の内装・砂漠の夕日・アラビアン×現代富裕層の過剰な美。" },
  { type: "mumbai",            label: "Mumbai",        moods: ["vivid", "pop", "art"] as Mood[],                   bgPlace: "gallery",       outfitStyle: "dress",        hint: "ボリウッド映画の美学。鮮烈なカラー・花装飾・インドの豊かな色彩文化・祝祭的なビジュアル。" },
  { type: "seoul_night",       label: "ソウルナイト",  moods: ["tiktok", "vivid", "sns_pop"] as Mood[],            bgPlace: "night_amusement",outfitStyle: "street",      hint: "ソウルのナイトシーン。ホンデ・明洞のネオン・K-popアイドル文化とストリートの交差。" },
  { type: "osaka_town",        label: "大阪下町",      moods: ["retro", "pop", "cinematic"] as Mood[],             bgPlace: "old_cinema",    outfitStyle: "street",       hint: "大阪の下町情緒。道頓堀・看板・人情と現代が混じり合う独特の賑わいと空気感。" },
  { type: "bangkok",           label: "Bangkok",       moods: ["mystic", "vivid", "art"] as Mood[],                bgPlace: "japanese_room", outfitStyle: "wa_modern",    hint: "バンコクの寺院と現代都市の共存。黄金の仏塔・熱帯の光・仏教文化×現代タイのコントラスト。" },
  { type: "sao_paulo",         label: "São Paulo",     moods: ["vivid", "art", "street"] as Mood[],                bgPlace: "industrial",    outfitStyle: "street",       hint: "サンパウロのアーバンアート。巨大なグラフィティ・熱帯の色彩・ブラジルのストリートカルチャー。" },
];

export function getCultureLabel(type: CultureType): string {
  return CULTURE_DIRECTIONS.find((d) => d.type === type)?.label ?? type;
}

export function buildCultureInputs(current: PromptInputs, _memory: VariationMemory = createEmptyMemory(), type: CultureType): PromptInputs {
  const dir = CULTURE_DIRECTIONS.find((d) => d.type === type)!;
  const note = [
    `【🌐 カルチャー — ${dir.label}】`, dir.hint, "",
    "▼ カルチャープリセットの方針：",
    "  - 選ばれた都市・文化圏の美意識・広告感・街の空気感を画面に落とし込む",
    "  - 文化的ステレオタイプに頼らず、その都市の「本物のかっこよさ」を表現する",
    "  - 各案で時間帯・構図・色調を変えて差別化する",
    "【絶対維持】顔・表情・人物の同一性・体型は一切変更しない。",
  ].join("\n");
  return {
    ...current, scopes: scopeOrDefault(current, pickScopesWeighted(PW.street, dir.moods, _memory.lastScopes)), moods: dir.moods, faceLock: true, viralMode: false, autoMoodCategories: [], extraInstructions: note,
    details: {
      ...current.details,
      outfit:     { ...current.details.outfit,     style: dir.outfitStyle },
      background: { ...DEFAULT_DETAILS.background, place: dir.bgPlace },
...
/**
 * 複数の SNS タイプを融合して PromptInputs を生成する。
 * - 1つ → 通常ビルダー
 * - 複数 → スコープ・ムードをマージしてコンボ指示文を生成
 */
export function buildCombinedSnsInputs(
  current: PromptInputs,
  types: SnsType[],
  memory: VariationMemory,
): PromptInputs {
  if (types.length === 0) return current;
  if (types.length === 1) return buildSnsInputs(current, memory, types[0]);

  const built = types.map((t) => buildSnsInputs(current, memory, t));
  const mergedScopes = [...new Set(built.flatMap((b) => b.scopes))] as Scope[];
  const mergedMoods  = [...new Set(built.flatMap((b) => b.moods))].slice(0, 6) as Mood[];
  const labels = types.map((t) => getSnsLabel(t));
  const notes  = built.map((b) => b.extraInstructions ?? "").filter(Boolean).join("\n\n");
  ...
  return {
    ...
    viralMode:          false,
    autoMoodCategories: [],
    extraInstructions:  `【📈 SNSコンボ：${labels.join(" × ")}】\n両方のプラットフォームで最大効果を発揮するビジュアルを作ること。\n\n${notes}`,
    details:            DEFAULT_DETAILS,
    count:              4 as Count,
  };
}
...
/**
 * 複数のカルチャータイプを融合して PromptInputs を生成する。
 */
export function buildCombinedCultureInputs(
  current: PromptInputs,
  types: CultureType[],
  memory: VariationMemory,
): PromptInputs {
  if (types.length === 0) return current;
  if (types.length === 1) return buildCultureInputs(current, memory, types[0]);

  const built = types.map((t) => buildCultureInputs(current, memory, t));
  const mergedScopes = [...new Set(built.flatMap((b) => b.scopes))] as Scope[];
  const mergedMoods  = [...new Set(built.flatMap((b) => b.moods))].slice(0, 6) as Mood[];
  const labels = types.map((t) => getCultureLabel(t));
  const notes  = built.map((b) => b.extraInstructions ?? "").filter(Boolean).join("\n\n");
  ...
  return {
    ...
    viralMode:          false,
    autoMoodCategories: [],
    extraInstructions:  `【🌐 カルチャーコンボ：${labels.join(" × ")}】\n両都市・文化圏の美意識を自然に融合させること。\n\n${notes}`,
  };
}
