...
const NOVEL_BG_DIRECTIONS: Record<BgPreset, NovelBgDirection> = {
  code_space: {
    ...
    note: [
      "【🖥 斬新背景：コード空間】",
      "背景を「流れるソースコード・端末グリフ・無数のデータライン」で画面いっぱいに埋め尽くす、未知のデジタル空間にする。",
      "実在の都市・看板・ロゴは使わず、どこにも存在しないコードの世界として描く。",
      "",
      "▼ 奥行き：文字・コードを前景から奥へ層を成して羅列し、遠近感のある深い空間にする（近景は大きく粗く、遠景は細かく霞ませる）。",
      ...
      "",
      "▼ 守ること：",
      "  × 顔の上に読める文字を大きく重ねない（同一性を保つ）。文字・コードは空間・前景側に置く。",
      "  × 実在の場所・ブランド・ロゴを出さない",
  ...
  math_world: {
    ...
    note: [
      "【🔢 斬新背景：数式世界】",
      "背景を「浮遊する数式・記号・幾何学的なライン」で画面いっぱいに敷き詰めた、未知の抽象空間にする。",
      "黒板的な虚空／無限に続く数式空間として描き、実在の場所は使わない。",
      "",
      "▼ 奥行き：数式・記号を前景から奥へ層を成して浮遊させ、遠近感のある深い空間にする（手前は大きく、奥は細かく無数に続かせる）。",
      ...
      "",
      "▼ 方向性（案ごとに差別化）：",
      "  浮遊する方程式 / 幾何学グリッド / 発光する記号 / 抽象的な座標空間 / 流れる数式",
      "",
      "▼ 守ること：",
      "  × 顔の上に読める数式を大きく重ねない（同一性を保つ）。数式・記号は空間・前景側に置く。",
      "  × 実在の場所・ブランド・ロゴを出さない",
      "【絶対維持】顔・表情・人物の同一性・体型・ポーズ・カメラ構図・衣装・露出は一切変更しない（斬新化するのは背景の質感・空気・ライティングのみ）。",
    ].join("\n"),
  },
