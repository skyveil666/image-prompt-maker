import { createContext, useContext, Suspense, lazy, useRef, useState } from "react";
...
import { FIELD_OPTIONS, isTagNg } from "../data/tagNgOptions";
...
// ─── per-tag NG（タグ個別NG）context ───────────────────────────────────────────
// 操作：タグを「ダブルクリック」でNG化／再ダブルクリックで解除（モード概念なし）。
// シングルクリックは従来どおり値選択。tagNg は永続（"category.field:value"）。
// FieldSection/MultiFieldSection が consume し、各セルの onDoubleClick で onToggleTagNg を呼ぶ。
interface NgModeCtxValue {
  tagNg: string[];
  onToggleTagNg: (fieldKey: string, value: string) => void;
}
const NgModeCtx = createContext<NgModeCtxValue>({
  tagNg: [],
  onToggleTagNg: () => {},
});
        ...
        {options.map((opt) => {
          ...
          return (
            <GridCell
              ...
              jaLabel={opt.label}
              active={value === opt.id}
              cellKind={optNg ? "ng" : "value"}
              title={opt.ja}
              onClick={() => onChange(value === opt.id ? "skip" : opt.id)}
              onDoubleClick={canNg ? () => {
                // ダブルクリック＝NGトグル。直前のシングルクリックで値選択された分も含め、このタグが値選択中なら外す（矛盾防止）。
                const sel = value === opt.id;
                ng.onToggleTagNg(fieldKey!, opt.id);
                if (sel) onChange("skip");
              } : undefined}
            />
          );
        })}
        ...
        {options.map((opt) => {
          ...
          return (
            <GridCell
              ...
              onDoubleClick={canNg ? () => {
                // ダブルクリック＝NGトグル。直前のシングルクリックで選択された分も含め、このタグが選択中なら外す（矛盾防止）。
                const sel = activeSet.includes(opt.id);
                ng.onToggleTagNg(fieldKey!, opt.id);
                if (sel) handleToggle(opt.id);
              } : undefined}
            />
          );
        })}
...
function HairContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      <FieldSection label="スタイル系統" fieldKey="hair.hairStyle" value={d.hair.hairStyle ?? "skip"} options={HAIR_STYLES} noTopMargin
        onChange={(v) => upd("hair", { hairStyle: v as DetailSettings["hair"]["hairStyle"] })} />
      <FieldSection label="長さ" fieldKey="hair.length" value={d.hair.length} options={HAIR_LENGTHS}
        onChange={(v) => upd("hair", { length: v as DetailSettings["hair"]["length"] })} />
      <MultiFieldSection label="形" fieldKey="hair.shape" singleValue={d.hair.shape} multiValues={d.multiOverrides?.["hair.shape"] ?? []} options={HAIR_SHAPES}
        onChange={mc("hair.shape", "shape")} />
      <MultiFieldSection label="髪色" fieldKey="hair.color" singleValue={d.hair.color} multiValues={d.multiOverrides?.["hair.color"] ?? []} options={HAIR_COLORS}
        onChange={mc("hair.color", "color")} />
      <MultiFieldSection label="質感" fieldKey="hair.texture" singleValue={d.hair.texture} multiValues={d.multiOverrides?.["hair.texture"] ?? []} options={HAIR_TEXTURES}
        onChange={mc("hair.texture", "texture")} />
      <FieldSection label="カラーモード" fieldKey="hair.colorMode" value={d.hair.colorMode} options={HAIR_COLOR_MODES}
        onChange={(v) => upd("hair", { colorMode: v as DetailSettings["hair"]["colorMode"] })} />
      <FieldSection label="前髪" fieldKey="hair.bangs" value={d.hair.bangs} options={HAIR_BANGS}
        onChange={(v) => upd("hair", { bangs: v as DetailSettings["hair"]["bangs"] })} />
      <MultiFieldSection label="毛先" fieldKey="hair.tips" singleValue={d.hair.tips} multiValues={d.multiOverrides?.["hair.tips"] ?? []} options={HAIR_TIPS}
        onChange={mc("hair.tips", "tips")} />
      <FieldSection label="ボリューム" fieldKey="hair.volume" value={d.hair.volume} options={HAIR_VOLUMES}
        onChange={(v) => upd("hair", { volume: v as DetailSettings["hair"]["volume"] })} />
      <MultiFieldSection label="アクセサリー" fieldKey="hair.accessory" singleValue={d.hair.accessory} multiValues={d.multiOverrides?.["hair.accessory"] ?? []} options={HAIR_ACCESSORIES}
...
function OutfitContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      ...
      <FieldSection label="露出" fieldKey="outfit.exposure" value={d.outfit.exposure} options={OUTFIT_EXPOSURES}
        onChange={(v) => upd("outfit", { exposure: v as DetailSettings["outfit"]["exposure"] })} />
      <FieldSection label="装飾量" fieldKey="outfit.decoration" value={d.outfit.decoration} options={OUTFIT_DECORATIONS}
        onChange={(v) => upd("outfit", { decoration: v as DetailSettings["outfit"]["decoration"] })} />
      <FieldSection label="季節感" fieldKey="outfit.season" value={d.outfit.season} options={OUTFIT_SEASONS}
        onChange={(v) => upd("outfit", { season: v as DetailSettings["outfit"]["season"] })} />
      <FieldSection label="高級感" fieldKey="outfit.luxury" value={d.outfit.luxury} options={OUTFIT_LUXURIES}
...
function BackgroundContent({ d, upd, chg }: {
  ...
  return (
    <div>
      ...
      <MultiFieldSection label="背景スタイル" fieldKey="background.style" singleValue={d.background.style ?? "skip"} multiValues={d.multiOverrides?.["background.style"] ?? []} options={BG_STYLES}
        onChange={mc("background.style", "style")} />
      <MultiFieldSection label="場所" fieldKey="background.place" singleValue={d.background.place} multiValues={d.multiOverrides?.["background.place"] ?? []} options={BG_PLACES}
        onChange={(single, multi) => {
          const mo = { ...(d.multiOverrides ?? {}) };
      ...
      <MultiFieldSection label="色" fieldKey="background.color" singleValue={d.background.color} multiValues={d.multiOverrides?.["background.color"] ?? []} options={BG_COLORS}
        onChange={mc("background.color", "color")} />
      <FieldSection label="時間帯" fieldKey="background.time" value={d.background.time} options={BG_TIMES}
        onChange={(v) => upd("background", { time: v as DetailSettings["background"]["time"] })} />
      <FieldSection label="天候" fieldKey="background.weather" value={d.background.weather} options={BG_WEATHERS}
        onChange={(v) => upd("background", { weather: v as DetailSettings["background"]["weather"] })} />
      <FieldSection label="密度" fieldKey="background.density" value={d.background.density} options={BG_DENSITIES}
        onChange={(v) => upd("background", { density: v as DetailSettings["background"]["density"] })} />
      <MultiFieldSection label="空間効果" fieldKey="background.effect" singleValue={d.background.effect} multiValues={d.multiOverrides?.["background.effect"] ?? []} options={BG_EFFECTS}
        onChange={mc("background.effect", "effect")} />
      <FieldSection label="奥行き" fieldKey="background.depth" value={d.background.depth} options={BG_DEPTHS}
      ...
      {textOpen && (
        <div className="mt-1 pl-1 border-l-2 border-emerald-500/20">
          <FieldSection label="文字の種類" fieldKey="background.textType" value={d.background.textType} options={BG_TEXT_TYPES} noTopMargin
            onChange={(v) => upd("background", { textType: v as DetailSettings["background"]["textType"] })} />
          <FieldSection label="雰囲気" fieldKey="background.textMood" value={d.background.textMood} options={BG_TEXT_MOODS}
            onChange={(v) => upd("background", { textMood: v as DetailSettings["background"]["textMood"] })} />
          <FieldSection label="配置" fieldKey="background.textLayout" value={d.background.textLayout} options={BG_TEXT_LAYOUTS}
            onChange={(v) => upd("background", { textLayout: v as DetailSettings["background"]["textLayout"] })} />
          <FieldSection label="質感" fieldKey="background.textTexture" value={d.background.textTexture} options={BG_TEXT_TEXTURES}
...
function PoseContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      <FieldSection label="種類" fieldKey="pose.type" value={d.pose.type} options={POSE_TYPES} noTopMargin
        onChange={(v) => upd("pose", { type: v as DetailSettings["pose"]["type"] })} />
      <MultiFieldSection label="印象" fieldKey="pose.impression" singleValue={d.pose.impression} multiValues={d.multiOverrides?.["pose.impression"] ?? []} options={POSE_IMPRESSIONS}
        onChange={mc("pose.impression", "impression")} />
      <FieldSection label="視線" fieldKey="pose.gaze" value={d.pose.gaze} options={POSE_GAZES}
        onChange={(v) => upd("pose", { gaze: v as DetailSettings["pose"]["gaze"] })} />
      <MultiFieldSection label="手の位置" fieldKey="pose.hand" singleValue={d.pose.hand} multiValues={d.multiOverrides?.["pose.hand"] ?? []} options={POSE_HANDS}
        onChange={mc("pose.hand", "hand")} />
      <MultiFieldSection label="足の位置" fieldKey="pose.foot" singleValue={d.pose.foot} multiValues={d.multiOverrides?.["pose.foot"] ?? []} options={POSE_FEET}
        onChange={mc("pose.foot", "foot")} />
      <FieldSection label="重心" fieldKey="pose.balance" value={d.pose.balance} options={POSE_BALANCES}
        onChange={(v) => upd("pose", { balance: v as DetailSettings["pose"]["balance"] })} />
      <MultiFieldSection label="動き" fieldKey="pose.motion" singleValue={d.pose.motion} multiValues={d.multiOverrides?.["pose.motion"] ?? []} options={POSE_MOTIONS}
        onChange={mc("pose.motion", "motion")} />
      <FieldSection label="体の向き" fieldKey="pose.orientation" value={d.pose.orientation} options={POSE_ORIENTATIONS}
...
function CameraContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      ...
      <MultiFieldSection label="角度" fieldKey="camera.angle" singleValue={d.camera.angle} multiValues={d.multiOverrides?.["camera.angle"] ?? []} options={CAMERA_ANGLES}
        onChange={mc("camera.angle", "angle")} />
      <MultiFieldSection label="距離" fieldKey="camera.distance" singleValue={d.camera.distance} multiValues={d.multiOverrides?.["camera.distance"] ?? []} options={CAMERA_DISTANCES}
        onChange={mc("camera.distance", "distance")} />
      <MultiFieldSection label="レンズ感" fieldKey="camera.lens" singleValue={d.camera.lens} multiValues={d.multiOverrides?.["camera.lens"] ?? []} options={CAMERA_LENSES}
        onChange={mc("camera.lens", "lens")} />
      <MultiFieldSection label="構図" fieldKey="camera.composition" singleValue={d.camera.composition} multiValues={d.multiOverrides?.["camera.composition"] ?? []} options={CAMERA_COMPOSITIONS}
        onChange={mc("camera.composition", "composition")} />
      <FieldSection label="画角" fieldKey="camera.fov" value={d.camera.fov} options={CAMERA_FOVS}
        onChange={(v) => upd("camera", { fov: v as DetailSettings["camera"]["fov"] })} />
      <FieldSection label="視点高さ" fieldKey="camera.eyeHeight" value={d.camera.eyeHeight} options={CAMERA_EYE_HEIGHTS}
...
function PropsContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      <MultiFieldSection label="カテゴリ" fieldKey="props.category" singleValue={d.props.category} multiValues={d.multiOverrides?.["props.category"] ?? []} options={PROPS_CATEGORIES} noTopMargin
        onChange={(single, multi) => {
          const mo = { ...(d.multiOverrides ?? {}) };
      ...
      <MultiFieldSection label="持たせ方" fieldKey="props.hold" singleValue={d.props.hold} multiValues={d.multiOverrides?.["props.hold"] ?? []} options={PROPS_HOLDS}
        onChange={mc("props.hold", "hold")} />
      <FieldSection label="サイズ" fieldKey="props.size" value={d.props.size} options={PROPS_SIZES}
        onChange={(v) => upd("props", { size: v as DetailSettings["props"]["size"] })} />
      <FieldSection label="光り方" fieldKey="props.glow" value={d.props.glow} options={PROPS_GLOWS}
        onChange={(v) => upd("props", { glow: v as DetailSettings["props"]["glow"] })} />
      <MultiFieldSection label="雰囲気" fieldKey="props.vibe" singleValue={d.props.vibe} multiValues={d.multiOverrides?.["props.vibe"] ?? []} options={PROPS_VIBES}
        onChange={mc("props.vibe", "vibe")} />
      <MultiFieldSection label="配置" fieldKey="props.placement" singleValue={d.props.placement} multiValues={d.multiOverrides?.["props.placement"] ?? []} options={PROPS_PLACEMENTS}
        onChange={mc("props.placement", "placement")} />
      <FieldSection label="個数" fieldKey="props.count" value={d.props.count} options={PROPS_COUNTS}
...
function BigObjectContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      ...
      <FieldSection label="種類" fieldKey="bigObject.type" value={d.bigObject.type} options={BIG_OBJECT_TYPES}
        onChange={(v) => upd("bigObject", { type: v as DetailSettings["bigObject"]["type"] })} />
      <MultiFieldSection label="状態" fieldKey="bigObject.condition" singleValue={d.bigObject.condition} multiValues={d.multiOverrides?.["bigObject.condition"] ?? []} options={BIG_OBJECT_CONDITIONS}
        onChange={mc("bigObject.condition", "condition")} />
      <FieldSection label="配置" fieldKey="bigObject.placement" value={d.bigObject.placement} options={BIG_OBJECT_PLACEMENTS}
        onChange={(v) => upd("bigObject", { placement: v as DetailSettings["bigObject"]["placement"] })} />
      <FieldSection label="サイズ" fieldKey="bigObject.size" value={d.bigObject.size} options={BIG_OBJECT_SIZES}
        onChange={(v) => upd("bigObject", { size: v as DetailSettings["bigObject"]["size"] })} />
      <MultiFieldSection label="雰囲気" fieldKey="bigObject.mood" singleValue={d.bigObject.mood} multiValues={d.multiOverrides?.["bigObject.mood"] ?? []} options={BIG_OBJECT_MOODS}
...
function VehicleContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      ...
      <FieldSection label="ジャンル" fieldKey="vehicle.genre" value={d.vehicle.genre} options={VEHICLE_GENRES} noTopMargin
        onChange={(v) => upd("vehicle", { genre: v as DetailSettings["vehicle"]["genre"] })} />
      <FieldSection label="種類" fieldKey="vehicle.type" value={d.vehicle.type} options={VEHICLE_TYPES}
        onChange={(v) => upd("vehicle", { type: v as DetailSettings["vehicle"]["type"] })} />
      <FieldSection label="関わり方" fieldKey="vehicle.interaction" value={d.vehicle.interaction} options={VEHICLE_INTERACTIONS}
        onChange={(v) => upd("vehicle", { interaction: v as DetailSettings["vehicle"]["interaction"] })} />
      <FieldSection label="時代感" fieldKey="vehicle.era" value={d.vehicle.era} options={VEHICLE_ERAS}
        onChange={(v) => upd("vehicle", { era: v as DetailSettings["vehicle"]["era"] })} />
      <MultiFieldSection label="素材感" fieldKey="vehicle.material" singleValue={d.vehicle.material} multiValues={d.multiOverrides?.["vehicle.material"] ?? []} options={VEHICLE_MATERIALS}
        onChange={mc("vehicle.material", "material")} />
      <MultiFieldSection label="雰囲気" fieldKey="vehicle.atmosphere" singleValue={d.vehicle.atmosphere} multiValues={d.multiOverrides?.["vehicle.atmosphere"] ?? []} options={VEHICLE_ATMOSPHERES}
...
function MythContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      ...
      <MultiFieldSection label="神話地域" fieldKey="myth.region" singleValue={d.myth.region} multiValues={d.multiOverrides?.["myth.region"] ?? []} options={MYTH_REGIONS} noTopMargin
        onChange={mc("myth.region", "region")} />
      <MultiFieldSection label="幻獣種別" fieldKey="myth.creature" singleValue={d.myth.creature} multiValues={d.multiOverrides?.["myth.creature"] ?? []} options={MYTH_CREATURES}
        onChange={mc("myth.creature", "creature")} />
      <MultiFieldSection label="配置・関わり方" fieldKey="myth.interaction" singleValue={d.myth.interaction} multiValues={d.multiOverrides?.["myth.interaction"] ?? []} options={MYTH_INTERACTIONS}
        onChange={mc("myth.interaction", "interaction")} />
      <MultiFieldSection label="描写スタイル" fieldKey="myth.style" singleValue={d.myth.style} multiValues={d.multiOverrides?.["myth.style"] ?? []} options={MYTH_STYLES}
        onChange={mc("myth.style", "style")} />
      <FieldSection label="サイズ感" fieldKey="myth.size" value={d.myth.size} options={MYTH_SIZES}
...
function LightingContent({ d, chg }: { d: DetailSettings; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      ...
      <MultiFieldSection label="光源方向" fieldKey="lighting.direction" singleValue={d.lighting.direction} multiValues={d.multiOverrides?.["lighting.direction"] ?? []} options={LIGHT_DIRECTIONS}
        onChange={mc("lighting.direction", "direction")} />
      <MultiFieldSection label="強さ" fieldKey="lighting.intensity" singleValue={d.lighting.intensity} multiValues={d.multiOverrides?.["lighting.intensity"] ?? []} options={LIGHT_INTENSITIES}
        onChange={mc("lighting.intensity", "intensity")} />
      <MultiFieldSection label="色温度" fieldKey="lighting.temperature" singleValue={d.lighting.temperature} multiValues={d.multiOverrides?.["lighting.temperature"] ?? []} options={LIGHT_TEMPS}
        onChange={mc("lighting.temperature", "temperature")} />
      <MultiFieldSection label="影" fieldKey="lighting.shadow" singleValue={d.lighting.shadow} multiValues={d.multiOverrides?.["lighting.shadow"] ?? []} options={LIGHT_SHADOWS}
        onChange={mc("lighting.shadow", "shadow")} />
      <MultiFieldSection label="反射" fieldKey="lighting.reflection" singleValue={d.lighting.reflection} multiValues={d.multiOverrides?.["lighting.reflection"] ?? []} options={LIGHT_REFLECTIONS}
        onChange={mc("lighting.reflection", "reflection")} />
      <MultiFieldSection label="空気感" fieldKey="lighting.atmosphere" singleValue={d.lighting.atmosphere} multiValues={d.multiOverrides?.["lighting.atmosphere"] ?? []} options={LIGHT_ATMOSPHERES}
...
function CyberContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      ...
      <MultiFieldSection label="変化する部位" fieldKey="cyber.part" singleValue={d.cyber.part} multiValues={d.multiOverrides?.["cyber.part"] ?? []} options={CYBER_PARTS} noTopMargin
        onChange={mc("cyber.part", "part")} />
      <MultiFieldSection label="機械化タイプ" fieldKey="cyber.type" singleValue={d.cyber.type} multiValues={d.multiOverrides?.["cyber.type"] ?? []} options={CYBER_TYPES}
        onChange={mc("cyber.type", "type")} />
      <MultiFieldSection label="質感" fieldKey="cyber.texture" singleValue={d.cyber.texture} multiValues={d.multiOverrides?.["cyber.texture"] ?? []} options={CYBER_TEXTURES}
        onChange={mc("cyber.texture", "texture")} />
      <MultiFieldSection label="発光色" fieldKey="cyber.glowColor" singleValue={d.cyber.glowColor} multiValues={d.multiOverrides?.["cyber.glowColor"] ?? []} options={CYBER_GLOW_COLORS}
...
function CosplayContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      <MultiFieldSection label="ジャンル系統" fieldKey="cosplay.genre" singleValue={d.cosplay.genre} multiValues={d.multiOverrides?.["cosplay.genre"] ?? []} options={COSPLAY_GENRES} noTopMargin
        onChange={mc("cosplay.genre", "genre")} />
      <FieldSection label="かわいい系" fieldKey="cosplay.cuteStyle" value={d.cosplay.cuteStyle} options={COSPLAY_CUTE_STYLES}
        onChange={(v) => upd("cosplay", { cuteStyle: v as DetailSettings["cosplay"]["cuteStyle"] })} />
      <FieldSection label="職種・役割系" fieldKey="cosplay.jobGenre" value={d.cosplay.jobGenre} options={COSPLAY_JOB_GENRES}
        onChange={(v) => upd("cosplay", { jobGenre: v as DetailSettings["cosplay"]["jobGenre"] })} />
      <FieldSection label="和風系" fieldKey="cosplay.japaneseStyle" value={d.cosplay.japaneseStyle} options={COSPLAY_JAPANESE_STYLES}
        onChange={(v) => upd("cosplay", { japaneseStyle: v as DetailSettings["cosplay"]["japaneseStyle"] })} />
      <FieldSection label="ファンタジー系" fieldKey="cosplay.fantasyStyle" value={d.cosplay.fantasyStyle} options={COSPLAY_FANTASY_STYLES}
        onChange={(v) => upd("cosplay", { fantasyStyle: v as DetailSettings["cosplay"]["fantasyStyle"] })} />
      <FieldSection label="SF・近未来系" fieldKey="cosplay.scifiStyle" value={d.cosplay.scifiStyle} options={COSPLAY_SCIFI_STYLES}
        onChange={(v) => upd("cosplay", { scifiStyle: v as DetailSettings["cosplay"]["scifiStyle"] })} />
      <FieldSection label="ダーク系" fieldKey="cosplay.darkStyle" value={d.cosplay.darkStyle} options={COSPLAY_DARK_STYLES}
        onChange={(v) => upd("cosplay", { darkStyle: v as DetailSettings["cosplay"]["darkStyle"] })} />
      <FieldSection label="職業コスプレ" fieldKey="cosplay.occupation" value={d.cosplay.occupation} options={COSPLAY_OCCUPATIONS}
        onChange={(v) => upd("cosplay", { occupation: v as DetailSettings["cosplay"]["occupation"] })} />
      <FieldSection label="装飾レベル" fieldKey="cosplay.decoration" value={d.cosplay.decoration} options={COSPLAY_DECORATIONS}
        onChange={(v) => upd("cosplay", { decoration: v as DetailSettings["cosplay"]["decoration"] })} />
      <MultiFieldSection label="持ち物・小物" fieldKey="cosplay.item" singleValue={d.cosplay.item} multiValues={d.multiOverrides?.["cosplay.item"] ?? []} options={COSPLAY_ITEMS}
      ...
      <MultiFieldSection label="カラー方向" fieldKey="cosplay.colorDir" singleValue={d.cosplay.colorDir} multiValues={d.multiOverrides?.["cosplay.colorDir"] ?? []} options={COSPLAY_COLOR_DIRS}
...
function ForegroundContent({ d, upd, chg }: { d: DetailSettings; upd: Updater; chg: (n: DetailSettings) => void }) {
  ...
  return (
    <div>
      ...
      <FieldSection label="プリセット" fieldKey="foreground.preset" value={d.foreground.preset} options={FG_PRESETS} noTopMargin
        onChange={(v) => upd("foreground", { preset: v as DetailSettings["foreground"]["preset"] })} />
      <MultiFieldSection label="エフェクト種類" fieldKey="foreground.effectType" singleValue={d.foreground.effectType} multiValues={d.multiOverrides?.["foreground.effectType"] ?? []} options={FG_EFFECT_TYPES}
        onChange={mc("foreground.effectType", "effectType")} />
      <MultiFieldSection label="回転・渦" fieldKey="foreground.swirlType" singleValue={d.foreground.swirlType} multiValues={d.multiOverrides?.["foreground.swirlType"] ?? []} options={FG_SWIRL_TYPES}
        onChange={mc("foreground.swirlType", "swirlType")} />
      <MultiFieldSection label="HUD・デジタル" fieldKey="foreground.digitalType" singleValue={d.foreground.digitalType} multiValues={d.multiOverrides?.["foreground.digitalType"] ?? []} options={FG_DIGITAL_TYPES}
        onChange={mc("foreground.digitalType", "digitalType")} />
      <MultiFieldSection label="アート表現" fieldKey="foreground.artType" singleValue={d.foreground.artType} multiValues={d.multiOverrides?.["foreground.artType"] ?? []} options={FG_ART_TYPES}
        onChange={mc("foreground.artType", "artType")} />
      <MultiFieldSection label="位置" fieldKey="foreground.position" singleValue={d.foreground.position} multiValues={d.multiOverrides?.["foreground.position"] ?? []} options={FG_POSITIONS}
        onChange={mc("foreground.position", "position")} />
      <FieldSection label="密度" fieldKey="foreground.density" value={d.foreground.density} options={FG_DENSITIES}
        onChange={(v) => upd("foreground", { density: v as DetailSettings["foreground"]["density"] })} />
      <MultiFieldSection label="動き" fieldKey="foreground.motion" singleValue={d.foreground.motion} multiValues={d.multiOverrides?.["foreground.motion"] ?? []} options={FG_MOTIONS}
        onChange={mc("foreground.motion", "motion")} />
      <MultiFieldSection label="色方向" fieldKey="foreground.color" singleValue={d.foreground.color} multiValues={d.multiOverrides?.["foreground.color"] ?? []} options={FG_COLORS}
        onChange={mc("foreground.color", "color")} />
      <FieldSection label="奥行き" fieldKey="foreground.depth" value={d.foreground.depth} options={FG_DEPTHS}
        onChange={(v) => upd("foreground", { depth: v as DetailSettings["foreground"]["depth"] })} />
      <FieldSection label="視認性" fieldKey="foreground.visibility" value={d.foreground.visibility} options={FG_VISIBILITIES}
...
export function DetailsCard({
  ...
  return (
    <NgModeCtx.Provider value={{ tagNg, onToggleTagNg }}>
